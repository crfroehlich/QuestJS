...
export const lang = {
    ...
    aboutScript() {
        ...
        if (Quest.Settings.settings.additionalAbout !== undefined) {
            ...
            for (const key in Quest.Settings.settings.additionalAbout)
                Quest.IO.metamsg(`{i:${key}:} ${Quest.Settings.settings.additionalAbout[key]}`);
        }
    ...
    stop_posture: function (char) {
        if (!char.posture)
            return '';
        if (!char.postureFurniture && char.posture === 'standing')
            return '';
        const options = { char };
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    addArticle(item, type) {
        if (!type || (type != Quest.Utilities.DEFINITE && type != Quest.Utilities.INDEFINITE))
            return;
    ...
    addDefiniteArticle: function (item) {
        ...
        if (item.owner)
            return `${lang.getName(Quest.World.w[item.owner], { possessive: true })} `;
        if (item.defArticle)
    ...
    getName(item, options) {
        ...
        if (options.capital)
            s = Quest.Utilities.sentenceCase(s);
    ...
    // would return "you go".
    // The first letter is capitalised if 'capitalise' is true.
    pronounVerb(item, verb, options) {
        let s = `${item.pronouns.subjective} ${lang.conjugate(item, verb)}`;
        s = s.replace(/ +\'/, "'"); // yes this is a hack!
        return options.capitalise ? Quest.Utilities.sentenceCase(s) : s;
    },
    yes_regex: /^(y|yes)$/i,
    pronounVerbForGroup(item, verb, options) {
    ...
    warningsScript() {
        ...
        switch (typeof Quest.Settings.settings.warnings) {
            ...
            case 'string':
                ...
                break;
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'warnings' does not exist on type '{ perf... Remove this comment to see the full error message
            default: for (const el of Quest.Settings.settings.warnings)
                Quest.IO.metamsg(el);
        }
        return Quest.World.world.SUCCESS_NO_TURNSCRIPTS;
    },
