...
Quest.World.createRoom('steam_corridor', {
    ...
    south: new Quest.World.Exit('steam_corridor_duct', {
        ...
        simpleUse(char) {
            ...
            if (Quest.World.w.grating.scenery)
                return Quest.IO.falsemsg('It works in Hollywood... Mandy gives the grating a good pull... It is stuck solid. No way is she getting into the vents without something sharp and strong to prise the grating off the wall.');
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'defaultSimpleExitUse' does not exist on ... Remove this comment to see the full error message
            return Quest.Utilities.util.defaultSimpleExitUse(char, this);
        },
    }),
    ...
    west: new Quest.World.Exit('brass_dining_room', {
        simpleUse(char) {
            ...
            if (Quest.World.w.brass_dining_room.blocked())
                return Quest.IO.falsemsg('Mandy starts heading west, but the dining room is now so full of mannequins, she cannot get into it.');
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'defaultSimpleExitUse' does not exist on ... Remove this comment to see the full error message
            return Quest.Utilities.util.defaultSimpleExitUse(char, this);
        },
    }),
...
Quest.World.createItem('grating', Quest.Templates.TAKEABLE(), {
    ...
    openwith(options) {
        ...
        if (item !== Quest.World.w.crocodile_tooth)
            return Quest.IO.falsemsg('Mandy wonders if she could open the grating with {nm:item:the}. She shakes her head - no, that will not work.', { item });
        return this.take(options);
    },
    scenery: true,
    synonyms: ['grate'],
    testTake(options) {
        ...
        if (Quest.World.w.crocodile_tooth.loc === Quest.World.player.name)
            return Quest.IO.falsemsg('Mandy looks at the crocodile tooth. If that was bigger, she might be able to use it to lever the grating off the wall.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
...
Quest.World.createItem('Silver', Quest.NPC.NPC(), {
    ...
    agendaPatch() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'steam_corridor_duct' does not exist on t... Remove this comment to see the full error message
        if (Quest.World.currentLocation === Quest.World.w.steam_corridor_duct)
            Quest.IO.msg('There is no sign of a Silver; Mandy wonders if Patch is scaring them off.');
    },
    agendaUpendPot() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'steam_corridor_duct' does not exist on t... Remove this comment to see the full error message
        if (Quest.World.currentLocation === Quest.World.w.steam_corridor_duct && Quest.World.w.chamber_pot.underLeak) {
...
Quest.World.createRoom('upper_steam_hall', Quest.Templates.ROOM_SET('steam hall'), {
    afterEnter() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        if (this.silverSighting[this.visited % 20])
            Quest.IO.msg(this.silverSighting[this.visited % 20]);
    },
...
Quest.World.createRoom('lift', Quest.Templates.TRANSIT('out'), {
    ...
    testTransit(player, options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'Patch' does not exist on type '{}'.
        if (Quest.World.w.Patch.loc !== this.name)
            return true;
        if (this[this.transitDoorDir].name === 'steam_control_room')
            return true;
        if (this[this.transitDoorDir].name === 'upper_steam_hall' && options.button.transitDest !== 'steam_control_room')
            return true;
        if (this[this.transitDoorDir].name === 'steam_hall' && options.button.transitDest === 'steam_hall')
            return true;
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
...
Quest.World.createRoom('steam_control_room', {
    afterEnter() {
        ...
        if (Quest.World.w.Winfield_Malewicz.loc === undefined || Quest.World.w.Winfield_Malewicz.loc === this.name || Quest.World.w.Winfield_Malewicz.dead)
            return;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'Winfield_Malewicz' does not exist on typ... Remove this comment to see the full error message
...
Quest.World.createItem('Winfield_Malewicz', Quest.NPC.NPC(), {
    askOptions: [
        ...
        },
        {
            ...
            script() {
                ...
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'Winfield_Malewicz' does not exist on typ... Remove this comment to see the full error message
                s += Quest.Utilities.formatList(Quest.World.w.Winfield_Malewicz.songlist.map((el) => `{i:${el}}`), { lastJoiner: 'and' });
    ...
    kill() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'lounge' does not exist on type '{}'.
        if (Quest.World.currentLocation === Quest.World.w.lounge)
            return Quest.IO.falsemsg('Why would Mandy want to kill Dr Malewicz now?');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
