...
Quest.World.createItem('hatstand', Quest.Templates.SURFACE(), {
    ...
    testDropIn(options) {
        ...
        if (options.item !== Quest.World.w.floppy_hat)
            return Quest.IO.falsemsg('Mandy can only put hats on the hatstand - the clue is in the name.');
        return true;
    },
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 2.
...
Quest.World.createItem('clockwork_thespian', Quest.NPC.NPC(), Quest.Templates.CONTAINER(false), {
    ...
    nameModifierFunction(list) {
        list.length = 0;
        if (Quest.World.w.floppy_hat.loc === this.name)
            list.push('wearing a floppy hat');
        if (Quest.World.w.large_key.loc === this.name)
            list.push('a large key in his back');
    },
    npc: false,
    state: 0,
    ...
    testDropIn(options) {
        ...
        if (options.item !== Quest.World.w.large_key)
            return Quest.IO.falsemsg('That is not something you can put in the keyhole. The clue is in the word "keyhole".');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
        if (Quest.World.w.large_key.size > 6)
            return Quest.IO.falsemsg('The key is too large for the keyhole.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
        if (Quest.World.w.large_key.size < 6)
            return Quest.IO.falsemsg('The key is too small for the keyhole.');
        return true;
    },
    testTalk(text) {
        if (!this.npc)
            return Quest.IO.falsemsg(text ? Quest.lang.cannot_ask_about : Quest.lang.cannot_talk_to, { char: Quest.World.player, item: this, text });
        return true;
    },
    windup() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
