    ...
    up: new Quest.World.Exit('_', {
        ...
        simpleUse(char) {
            ...
            if (char === Quest.World.w.Patch) {
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
                if (Quest.World.w.grown_tamarind_tree.growthTime < 9)
                    return Quest.IO.falsemsg('Patch looks around, confused, wondering what he should be climbing.');
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'Patch' does not exist on type '{}'.
                if (Quest.World.w.Patch.huggingTree)
                    return Quest.IO.falsemsg("'Climb the stupid tree,' says Mandy. Patch looks forlornly up the tree he is hugging.");
                let s = 'Patch eyes up ';
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
            if (Quest.World.w.grown_tamarind_tree.seedsPlanted === 0)
                return Quest.IO.falsemsg('Mandy looks around, but there is nothing suitable to climb here.');
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
            if (Quest.World.w.hourglass.active)
                return Quest.IO.falsemsg('Mandy looks at the rapidly grow tamarind plant thoughtfully. If she grabs on to it now, it will take her up with it as it grows! Steps closer, and her foot starts to blur, and feel numb. A moment later she is feeling nauseated. She steps back, trying to catch her breath. Perhaps not such a great idea...');
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
            if (Quest.World.w.grown_tamarind_tree.growthTime < 9)
                return Quest.IO.falsemsg('Mandy looks at the {nm:item:false:false:count_this} thoughtfully. If {nv:item:were} more substantial, she could climb {ob:item}.', { count_this: 'seedsPlanted', item: Quest.World.w.grown_tamarind_tree });
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
            if (Quest.World.w.grown_tamarind_tree.seedsPlanted > 1)
                return Quest.IO.falsemsg('Mandy looks up at the trees; they are certainly high enough that if she could climb them, she could reach across to the top of the metal box, but they are too tall to climb. The lowest branches are way out of reach.');
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
...
Quest.Text.addDirective('floraFlowerBed', (arr, params) => {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
    if (Quest.World.w.grown_tamarind_tree.growthTime === 0) {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
        if (Quest.World.w.hourglass.loc === 'pedestal')
            s += ' There is {select:hourglass:size:::::a tiny:an:a large:} hourglass on the pedestal.';
        return s;
    }
...
// ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
Quest.Text.addDirective('floraDesc', (arr, params) => {
    ...
    if (Quest.World.w.greenhouse_west.seenFlag)
        return '';
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'greenhouse_west' does not exist on type ... Remove this comment to see the full error message
...
// ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
Quest.Text.addDirective('tamarind', (arr, params) => 
...
Quest.World.createItem('lift_fake_flora', {
    ...
    isLocatedAt(loc) {
        if (loc === 'greenhouse_catwalk_west' || loc === 'greenhouse_west')
            return true;
        return false;
    },
    scenery: true,
...
Quest.World.createItem('hourglass', SIZE_CHANGING(), {
    ...
    eventScript() {
        ...
        if (roundedState > 0) {
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
            if (this.loc === 'pedestal')
                Quest.World.w.grown_tamarind_tree.magicGrowth();
        }
        else {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            if (this.reporting())
                Quest.IO.msg('As time passes, sand falls from the upper bulb of the hourglass; it is now empty.');
            this.active = false;
            if (this.state < 0)
                this.state = 0;
            // Quest.World.w.grown_tamarind_tree.magicGrowthEnd()
    ...
    getIncrement() {
        if (this.size === 4)
            return 100;
        if (this.size === 5)
            return 100 / (this.fillState / 3 + 1);
        if (this.size === 6)
            return 100 / (this.fillState * 2 + 2);
    },
    handleInOutContainer(options, objects) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'protrusion' does not exist on type '{}'.
    ...
    */
    ...
    reporting() {
        if (this.loc === Quest.World.player.loc)
            return true;
        if (this.loc === Quest.World.player.name)
            return true;
        if (this.loc === 'pedestal' && Quest.World.player.loc === 'greenhouse_west')
            return true;
        return false;
    },
    // vessel:true,
    sink(fluid, char, vessel) {
        ...
        if (this.size < 7 && Quest.World.w.paper_funnel.loc !== 'protrusion')
            return Quest.IO.falsemsg('Mandy thinks about getting the sand into the hourglass... There is no way she is getting it though that narrow opening.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'paper_funnel' does not exist on type '{}... Remove this comment to see the full error message
        if (this.size < 5 && Quest.World.w.paper_funnel.loc === 'protrusion')
            return Quest.IO.falsemsg('Mandy thinks about getting the sand into the hourglass... There is no way she is getting it though that narrow opening, even with the paper funnel.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.size < 5)
            return Quest.IO.falsemsg('Mandy thinks about getting the sand into the hourglass... There is no way she is getting it though that narrow opening.');
        ...
        if (Quest.World.w.chamber_pot.size > 4)
            this.fillState += Quest.World.w.chamber_pot.size === 5 ? 1 : 5;
        if (this.fillState > 5)
            this.fillState = 5;
        let s;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'protrusion' does not exist on type '{}'.
    ...
    testFill(options) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.fillState > 4)
            return Quest.IO.falsemsg('The lower chamber of the hourglass is already full.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (Quest.World.w.chamber_pot.loc === Quest.World.player.name && Quest.World.w.chamber_pot.containedFluidName === 'sand')
            return true;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (Quest.World.w.chamber_pot.loc === Quest.World.player.name && Quest.World.w.chamber_pot.containedFluidName === 'water')
            return Quest.IO.falsemsg('Mandy thinks about pouring water into the hourglass; that would probably stop it working altogether, and it would be impossible to get the water out again. She decides not to.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (options.fluid === 'water')
            return Quest.IO.falsemsg('Mandy thinks about pouring water into the hourglass; that would probably stop it working altogether, and it would be impossible to get the water out again. She decides not to.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'beach' does not exist on type '{}'.
        if (Quest.World.currentLocation === Quest.World.w.beach || Quest.World.currentLocation === Quest.World.w.rocky_beach)
            return Quest.IO.falsemsg('Mandy considers filling the hourglass -- but she would need something to put the sand in first.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    ...
    turn() {
        ...
        if (Quest.World.w.grown_tamarind_tree.growthTime > 0) {
            ...
            if (Quest.Utilities.doOnce(this, 'flagWithered'))
                s += " 'Shit...' she mutters in disappointment.";
...
Quest.World.createItem('protrusion', Quest.Templates.COMPONENT('hourglass'), Quest.Templates.CONTAINER(true), {
    ...
    testDropIn(options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'paper_funnel' does not exist on type '{}... Remove this comment to see the full error message
        if (options.item !== Quest.World.w.paper_funnel)
            return Quest.IO.falsemsg('Mandy cannot put {nm:item:the} in the hourglass.', options);
        // okay if pf.s === hg.s + 1
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'paper_funnel' does not exist on type '{}... Remove this comment to see the full error message
        if (Quest.World.w.paper_funnel.size > this.size + 1)
            return Quest.IO.falsemsg('The paper funnel is too big to fit in the tap of the hourglass.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'paper_funnel' does not exist on type '{}... Remove this comment to see the full error message
        if (Quest.World.w.paper_funnel.size < this.size + 1)
            return Quest.IO.falsemsg('The paper funnel is too small to fit in the tap of the hourglass.');
        return true;
    },
});
// ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
Quest.Text.addDirective('hourglass', (arr, params) => {
    let s;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'paper_funnel' does not exist on type '{}... Remove this comment to see the full error message
    if (Quest.World.w.paper_funnel.loc === 'protrusion')
        s += ' There is a paper funnel in it.';
    return s;
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 3.
Quest.World.createItem('bare_earth', Quest.Templates.CONTAINER(), {
    ...
    testDropIn(options) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (options.item.alias === 'tamarind pod')
            return Quest.IO.falsemsg('Mandy wonders if burying the tamarind pod is going to achieve anything. She has a feeling she really needs to get the seeds out first.');
        if (options.item.name !== 'tamarind_seed')
            return Quest.IO.falsemsg('Mandy wonders if burying {nm:item:the} is going to achieve anything. Probably not.', { item: options.item });
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
        if (Quest.World.w.hourglass.active)
            return Quest.IO.falsemsg('Mandy starts to put another {nm:item:} in the ground, but as her hands get near, they start to blur, and feel numb. Perhaps not such a good idea when the hourglass is running.', { item: options.item, item_count: options.count });
        return true;
    },
    testTakeOut(options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
        if (Quest.World.w.hourglass.active)
            return Quest.IO.falsemsg('Mandy starts to dig {nm:item:the} from the ground, but as her hands get near, they start to blur, and feel numb. Perhaps not such a good idea when the hourglass is running.', { item: options.item, item_count: options.count });
        return true;
    },
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 2.
Quest.World.createItem('grown_tamarind_tree', {
    ...
    examine() {
        ...
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
        Quest.IO.msg(s, { count_this: 'seedsPlanted', item: Quest.World.w.grown_tamarind_tree });
    },
    getDescId() {
        let n = this.seedsPlanted - 1;
        if (this.seedsPlanted > 3)
            n = 2;
        if (this.seedsPlanted > 5)
            n = 3;
    ...
    magicGrowth() {
        if (this.seedsPlanted === 0)
            return;
        this.growthTime++;
        this.update();
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
        if (Quest.World.player.loc === 'greenhouse_west')
            Quest.IO.msg('Mandy watches as {nv:item:grow:false:count_this}; {nv:item:be:false:suppressCount} now about {show:grown_tamarind_tree:getHeight} high.', { count_this: 'seedsPlanted', item: Quest.World.w.grown_tamarind_tree, suppressCount: 'seedsPlanted' });
    },
    magicGrowthEnd() {
        if (this.seedsPlanted === 0)
            return;
...
Quest.World.createRoom('up_a_tall_tree', {
    ...
    southwest: new Quest.World.Exit('lift', {
        ...
        simpleUse(char) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'lift' does not exist on type '{}'.
            if (Quest.World.w.lift.getTransitDestLocation() !== Quest.World.w.steam_control_room)
                return Quest.IO.falsemsg('Mandy looks at the top of the metal box, and realises the lift is not there. It would be a bad idea heading that way, she decides.');
...
Quest.World.createItem('hourglass_from_catwalk', {
    ...
    isLocatedAt(loc) {
        if (loc !== 'greenhouse_catwalk_west')
            return false;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
        if (Quest.World.w.hourglass.loc !== 'pedestal')
            return false;
...
// ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
Quest.Text.addDirective('chamberPotUnderTree', (arr, params) => {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
    if (!Quest.World.w.chamber_pot.underTree)
        return '';
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
    if (Quest.World.w.chamber_pot.flipped)
        return ' There is an upturned chamber pot under the tree.';
...
Quest.World.createItem('tamarind_tree_from_ground', {
    ...
    kick(options) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (options.char === Quest.World.player)
            return Quest.IO.falsemsg('Mandy kicks the stupid tree in frustration.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
    ...
    shake(options) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (options.char === Quest.World.player)
            return Quest.IO.falsemsg('Mandy tries to shake the tree, but the trunk is too thick; it does not budge. If only she were higher up...');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('tamarind_tree', {
    ...
    take() {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'Patch' does not exist on type '{}'.
        if (Quest.World.w.Patch.loc === 'greenhouse_east' && Quest.World.w.Patch.hasPod())
            return Quest.IO.falsemsg('Patch looks very confused as he stares at the pod in his hand, then the pod on the ground, then the pod in his hand again. Then suddenly a silvery figure appears from the west, quickly scoops up the pod from the floor, and runs back west.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'Patch' does not exist on type '{}'.
        if (Quest.World.w.Patch.loc !== 'greenhouse_east')
            return Quest.IO.falsemsg("Suddenly a silvery figure appears from the west, quickly scoops up the pod from the floor, and runs back west.{once: 'Shit,' mutters Mandy.}");
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
...
Quest.World.createItem('tamarind_pod_prototype', SIZE_CHANGING(), {
    ...
    open(options) {
        ...
        if (options.secondItem === undefined) {
            ...
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            if (!sharp)
                return Quest.IO.falsemsg('Mandy tries to open the tamarind pod, but it is too tough for her fingernails. She needs something a little sharper.');
        }
        else {
            if (options.secondItem.npc)
                return Quest.IO.falsemsg('Mandy tries to open the pod with {nm:secondItem:the}, but {pv:secondItem:start} to get annoyed and moves away.', options);
            if (options.secondItem.loc !== Quest.World.player.name)
                return Quest.IO.falsemsg('Mandy thinks about opening the pod with {nm:secondItem:the}... She would have to be holding it, of course.', options);
            if (!options.secondItem.sharp)
                return Quest.IO.falsemsg('Mandy tries to open the pod with {nm:secondItem:the} for a few minutes before giving up in frustration.', options);
