    ...
    use() {
        ...
        if (this.azimuth === 4 && this.altitude === 0 && Quest.World.w.glass_shard.loc === 'controls') {
            ...
            if (Quest.World.w.glass_shard.size === 7) {
                ...
                if (this.shardTime === 8)
                    Quest.World.player.easterEgg = true;
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('left_wheel', {
    ...
    doTurn(inc) {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
        if (Quest.World.w.telescope.azimuth >= Quest.World.w.telescope.azimuths.length)
            Quest.World.w.telescope.azimuth = 0;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
        if (Quest.World.w.telescope.azimuth < 0)
            Quest.World.w.telescope.azimuth = Quest.World.w.telescope.azimuths.length - 1;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
...
for (const dir of Quest.World.w.telescope.azimuths) {
    ...
    Quest.World.w.observatory_up[dir] = new Quest.World.Exit('telescope_end', {
        ...
        simpleUse(char) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
            if (this.dir !== Quest.World.w.telescope.azimuths[Quest.World.w.telescope.azimuth])
                return Quest.IO.falsemsg(Quest.lang.not_that_way, { char, dir: this.dir });
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
            if (Quest.World.w.telescope.altitude > 4)
                return Quest.IO.falsemsg('Mandy looks at the end of the telescope; if it were not so steep and smooth, she could edge along it{if:telescope:azimuth:6:, and perhaps get out onto the roof to the west}.');
    ...
    Quest.World.w.telescope_end[dir] = new Quest.World.Exit('observatory_up', {
        isHidden() {
            ...
            if (this.dir === Quest.World.w.telescope.azimuths[Quest.World.w.telescope.azimuth])
                return false;
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
            if (Quest.World.w.telescope.roofOpen && this.dir === 'west' && Quest.World.w.telescope.azimuth === 6)
                return false;
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
        ...
        simpleUse(char) {
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
            if (!Quest.World.w.telescope.roofOpen && this.dir === Quest.World.w.telescope.azimuths[Quest.World.w.telescope.azimuth])
                return Quest.IO.falsemsg('Mandy looks at the slot in the ceiling, just beyond the end of the telescope. If that were open, she might be able to get out through it, she muses.');
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
            if (Quest.World.w.telescope.roofOpen && this.dir !== 'west' && this.dir === Quest.World.w.telescope.azimuths[Quest.World.w.telescope.azimuth])
                return Quest.IO.falsemsg('Mandy considers for a moment a leap of faith from the end of the telescope, out through the slot in the ceiling... No, not a good idea.');
            if (this.isHidden())
                return Quest.IO.falsemsg(Quest.lang.not_that_way, { char, dir: this.dir });
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'telescope' does not exist on type '{}'.
            if (this.dir === Quest.World.w.telescope.azimuths[(Quest.World.w.telescope.azimuth + 4) % 8])
                return Quest.Utilities.util.defaultSimpleExitUse(char, this);
