...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('Dance', {
    ...
    script() {
        ...
        if (Quest.World.w.Winfield_Malewicz.isHere())
            Quest.IO.msg("'The ways of young people were decidedly odd in my day,' says Malewicz. 'I see the situation has only deteriorated.'");
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'clockwork_thespian' does not exist on ty... Remove this comment to see the full error message
        if (Quest.World.w.clockwork_thespian.isHere())
            Quest.IO.msg("'Ah, the artistic soul,' muses the clockwork thespian.");
        return Quest.World.world.FAILED;
    },
}));
// ts-error-fixed ts-migrate(7009) FIXME: 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('ThrowAtPod', {
    ...
    script(objects) {
        ...
        if (Quest.World.player.loc !== 'greenhouse_east' && Quest.World.player.loc !== 'greenhouse_catwalk_east')
            return Quest.IO.failedmsg('Mandy would have to be holding {nm:item:the} to do that.', { item: obj });
        ...
        if (obj === Quest.World.w.mobile_phone)
            return Quest.IO.failedmsg('Mandy thinks about chucking her crappy phone at the pod, but if she does ever get out of here, she want a phone that is {i:not} smashed.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'school_bag' does not exist on type '{}'.
        if (obj === Quest.World.w.school_bag)
            return Quest.IO.failedmsg('Mandy thinks about throwing her bag at the pod, but decides she should hang on to it.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hourglass' does not exist on type '{}'.
        if (obj === Quest.World.w.hourglass)
            return Quest.IO.failedmsg('Mandy thinks about throwing the hourglass at the pod... She has a feeling it might be important, and is unlikely to survive its arboreal journey.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'china_doll' does not exist on type '{}'.
        if (obj === Quest.World.w.china_doll)
            return Quest.IO.failedmsg('Mandy thinks about throwing the china doll at the pod... Somehow she cannot bring herself to do it - seeing her smash would be too awful');
        if (obj.size && obj.size > 5)
            return Quest.IO.failedmsg('Mandy thinks about hurling {nm:item:the} at the tree. Perhaps if {pv:item:was} not so big...', { item: obj });
        const s = this.responses[obj.name];
        if (!s)
            return Quest.IO.failedmsg('Mandy thinks about throwing {nm:item:the} at the pod, but decides she should hang on to {sb:item}.', { item: obj });
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('NpcHelp', {
    ...
    script(objects) {
        const obj = objects[0][0];
        if (!obj.npc)
            return Quest.IO.falsemsg("'Give me a hand, here,' says Mandy to {nm:item:the}. {pv:item:do:true} not do anything.", { item: obj });
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'Patch' does not exist on type '{}'.
        if (obj !== Quest.World.w.Patch)
            return Quest.IO.falsemsg("'Give me a hand, here,' says Mandy to {nm:item:the}.|'What do you think I can do?' {pv:item:say:true}. Mandy shrugs, not sure herself.", { item: obj });
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('Plant', {
    ...
    script(objects) {
        ...
        if (Quest.World.player.loc !== 'greenhouse_west')
            return Quest.IO.failedmsg('There is no where for Mandy to plant anything here.');
        const obj = objects[0][0];
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (obj.name.startsWith('tamarind_pod_prototype'))
            return Quest.IO.failedmsg('Mandy thinks about burying the pod, but she is pretty sure her dad would take the seeds out first, and plant them, rather than the pods.');
        if (obj !== Quest.World.w.tamarind_seed)
            return Quest.IO.failedmsg("{pv:item:'be:true} not something Mandy really wants to bury.", { item: obj });
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('Invert', {
    ...
    script(objects) {
        const obj = objects[0][0];
        if (!obj.turn)
            return Quest.IO.failedmsg('Mandy thinks about turning {nm:item:the} upside-down, but decides it is fine the way it is.', { item: obj });
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('BurstBalloon', {
    ...
    script(objects) {
        ...
        if (!objects[0][0].sharp)
            return Quest.IO.failedmsg("Mandy's not going to pierce anything with that!");
        ...
        if (objects[1][0] === Quest.World.w.yellow_balloon)
            return Quest.World.w.yellow_balloon.burst() ? Quest.World.world.SUCCESS : Quest.World.world.FAILED;
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('BurstBalloonOnly', {
    ...
    script(objects) {
        ...
        if (obj === Quest.World.w.yellow_balloon)
            return Quest.World.w.yellow_balloon.burst() ? Quest.World.world.SUCCESS : Quest.World.world.FAILED;
        if (obj.smash)
            return objects[0][0].smash({ char: Quest.World.player, item: obj }) ? Quest.World.world.SUCCESS : Quest.World.world.FAILED;
    ...
    script(objects) {
        ...
        if (objects[1][0] === Quest.World.w.leaking_pipe) {
            ...
            if (objects[0][0] !== Quest.World.w.chamber_pot)
                return Quest.IO.falsemsg('Mandy thinks about putting {nm:item:the} under the leak... Then again, {pv:item:will} just get wet, so maybe not such a good idea.', { item: objects[0][0] });
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
            if (Quest.World.w.chamber_pot.containedFluidName)
                return Quest.IO.falsemsg(`The chamber pot is already full of ${Quest.World.w.chamber_pot.containedFluidName} - there is not much point using it to catch drips.`);
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        ...
        if (objects[1][0] === Quest.World.w.tamarind_tree_from_ground) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
            if (objects[0][0] !== Quest.World.w.chamber_pot)
                return Quest.IO.falsemsg('Mandy thinks about putting {nm:item:the} under the tamarind tree... but decides that is not going to achieve anything.', { item: objects[0][0] });
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.Command.findCmd('Say').script = function (objects) {
    ...
    if (Quest.World.currentLocation.name === 'weird_room') {
        ...
        if (text.match(/north|south|east|west/)) {
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'Winfield_Malewicz' does not exist on typ... Remove this comment to see the full error message
            if (Quest.World.w.Winfield_Malewicz.loc)
                Quest.IO.msg("'I'm not sure I follow your reasoning,' says Dr Malewicz. 'Is that a direction?'");
        }
        return Quest.World.world.SUCCESS;
    }
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'Patch' does not exist on type '{}'.
