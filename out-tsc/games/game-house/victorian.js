...
Quest.World.createRoom('front_hall', {
    ...
    north: new Quest.World.Exit('brass_dining_room', {
        use(char) {
            ...
            if (Quest.World.w.front_hall.exitState < 2)
                Quest.World.w.front_hall.exitState = 1;
    ...
    south: new Quest.World.Exit('gallery', {
        use(char) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'front_hall' does not exist on type '{}'.
            if (Quest.World.w.front_hall.exitState < 2)
                Quest.World.w.front_hall.exitState = 2;
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'defaultExitUse' does not exist on type '... Remove this comment to see the full error message
            return Quest.Utilities.util.defaultExitUse(char, this);
        },
    }),
...
Quest.World.createRoom('brass_dining_room', {
    ...
    desc() {
        ...
        if (this.mannequinCount > 0) {
            ...
            if (this.mannequinCount > 12)
                s += ' It is getting crowded in here!';
        }
...
Quest.World.createItem('clock', Quest.Templates.CONTAINER(false), {
    ...
    examine() {
        ...
        if (this.setTimeCount === 1)
            extra = ' What the fuck? She just set it to four o\'clock!';
        if (this.setTimeCount > 2 && this.setTimeCount % 2 === 1)
            extra = ' Again!';
    ...
    setclock() {
        ...
        if (this.setTimeCount % 2 === 0)
            this.setTimeCount++;
    },
    synonyms: ['time'],
    testDropIn(options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
        if (options.item !== Quest.World.w.large_key)
            return Quest.IO.falsemsg('That is not something you can put in a clock.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
        if (Quest.World.w.large_key.size > 5)
            return Quest.IO.falsemsg('The key is too large for the clock.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
        if (Quest.World.w.large_key.size < 5)
            return Quest.IO.falsemsg('The key is too small for the clock.');
        return true;
    },
    windup() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'large_key' does not exist on type '{}'.
...
Quest.World.createItem('dining_room_chair', Quest.Templates.FURNITURE({ sit: true }), {
    ...
    testPostureOn() {
        ...
        if (Quest.World.w.brass_dining_room.mannequinCount > 7)
            return Quest.IO.falsemsg('Mandy looks at the chairs; all are occupied by mannequins, preventing her from sitting on any of them.');
        return true;
    },
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 3.
Quest.World.createRoom('gallery', Quest.Templates.ROOM_SET('gallery'), {
    ...
    east: new Quest.World.Exit('_', {
        simpleUse(char) {
            ...
            if (!Quest.World.w.room_big.accessedFrom)
                Quest.World.w.room_big.accessedFrom = 'gallery_south';
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'room_big' does not exist on type '{}'.
    ...
    north: new Quest.World.Exit('brass_dining_room', {
        use(char) {
            ...
            if (Quest.World.w.brass_dining_room.blocked())
                return Quest.IO.falsemsg('Mandy starts heading north, but the dining room is now so full of mannequins, she cannot get into it.');
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'front_hall' does not exist on type '{}'.
            if (Quest.World.w.front_hall.exitState === 2) {
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('white_knight', {
    ...
    turn() {
        if (this.active) {
            ...
            if (Quest.World.w.Patch.leaderName)
                Quest.World.w.Patch.setLeader();
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createRoom('gallery_south', Quest.Templates.ROOM_SET('gallery'), {
    ...
    east: new Quest.World.Exit('_', {
        simpleUse(char) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'room_big' does not exist on type '{}'.
            if (!Quest.World.w.room_big.accessedFrom)
                Quest.World.w.room_big.accessedFrom = 'gallery';
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'room_big' does not exist on type '{}'.
...
Quest.World.createItem('mahogany_cabinet', Quest.Templates.OPENABLE(true), {
    ...
    push() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.moved)
            return Quest.IO.falsemsg("Mandy looks at the bureau. 'No, not shifting that thing again.'");
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createRoom('room_small', {
    ...
    in: new Quest.World.Exit('boots_room', {
        ...
        simpleUse(char) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'boots' does not exist on type '{}'.
            if (Quest.World.w.boots.loc !== 'room_small' || Quest.World.w.boots.size < 6)
                return Quest.IO.falsemsg(Quest.lang.not_that_way, { char, dir: this.dir });
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'defaultSimpleExitUse' does not exist on ... Remove this comment to see the full error message
...
Quest.World.createItem('boots', SIZE_CHANGING(), {
    ...
    enteritem() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'boots' does not exist on type '{}'.
        if (Quest.World.w.boots.size < 6)
            return Quest.IO.falsemsg('The boots are too small for Mandy to get inside.');
    ...
    repair(options) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.mended)
            return Quest.IO.falsemsg('Mandy looks at the boots -- they are probably as mended as they will ever be.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (options.char === Quest.World.player)
            return Quest.IO.falsemsg('Mandy looks at the boots. If she had some suitable materials, she could fix them. And the the right tools, of course. And some clue about what to do. Hmm, maybe she should find someone else to do it.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'tiny_man' does not exist on type '{}'.
        if (options.char !== Quest.World.w.tiny_man)
            Quest.IO.falsemsg('Mandy wonders if she could ask {nm:char:the} to fix the boots. She decides there is no points.');
        this.doRepair();
        return true;
    },
    scenery: true,
...
Quest.World.createItem('secret_recipe', SIZE_CHANGING(), {
    ...
    read() {
        ...
        if (this.size < 5)
            return falsemag('The writing is too small to read.');
        let s = '';
        if (this.size === 6) {
...
//# sourceMappingURL=victorian.js.map