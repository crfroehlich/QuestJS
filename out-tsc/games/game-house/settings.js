...
Quest.Settings.settings.roomCreateFunc = function (o) {
...
Quest.Settings.settings.customNoExitMsg = function (char, dir) {
    ...
    if (list.length === 1)
        return `She cannot go ${dir}. Looks like the only exit is back ${list[0]}.`;
...
Quest.Settings.settings.statsData = [
    ...
    {
        name: 'Locations requiring work',
        test(o) {
            return o.room && o.todo !== undefined;
        },
    },
    {
        name: 'Items',
...
Quest.Settings.settings.setup = function () {
    ...
    for (const s in Quest.World.w.shakespeare_book.names)
        Quest.World.w.shakespeare_book.synonyms.push(Quest.World.w.shakespeare_book.names[s]);
...
Quest.Settings.settings.betaTesters = [
    ...
    ['Jade', 'Mike Russo'],
    ['Karona', 'Zed Lopez'],
    ['ChristopherMerriner', 'Mike_G', 'Agnieszka Trzaska'],
    ['Amanda Walker'],
