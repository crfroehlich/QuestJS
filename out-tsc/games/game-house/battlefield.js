...
Quest.World.createItem('dead_horse', {
    ...
    withUse(char, obj) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'oar' does not exist on type '{}'.
        if (obj !== Quest.World.w.oar)
            return Quest.IO.falsemsg('Mandy wonder how she ould use {nm:item:the} on the horse.', { item: obj });
        return this.turn();
    },
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 2.
...
Quest.World.createItem('sea', Quest.Templates.CONTAINER(), {
    ...
    testDropIn(options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (options.item !== Quest.World.w.chamber_pot || Quest.World.w.chamber_pot.size !== 7)
            return Quest.IO.falsemsg("The thought of putting anything in that filthy water makes Mandy's stomach turn.");
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (Quest.World.w.chamber_pot.containedFluidName)
            return Quest.IO.falsemsg('Mandy wonders if the chamber pot would make a boat. Not while it is full of {show:fluid} of course.', { fluid: Quest.World.w.chamber_pot.containedFluidName });
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('sand_item', {
    ...
    play(options) {
        ...
        if (Quest.World.w.sandcastle.loc)
            return Quest.IO.falsemsg('Mandy looks at her sandcastle. Perhaps that is enough playing for now.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'sandcastle' does not exist on type '{}'.
    ...
    take(options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (!Quest.World.w.chamber_pot.isHeld())
            return Quest.IO.falsemsg('Mandy scoops up some sand in her hand, and watches as it slowly falls through her fingers.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (Quest.World.w.chamber_pot.containedFluidName === 'sand')
            return Quest.IO.falsemsg('Mandy things about getting more sand, but chamber pot is already full, she will not be able to carry more.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'chamber_pot' does not exist on type '{}'... Remove this comment to see the full error message
        if (Quest.World.w.chamber_pot.containedFluidName)
            return Quest.IO.falsemsg('Mandy thinks about getting some sand, but the chamber pot is already full; she would need to empty it first.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('sandcastle', Quest.Templates.CONSTRUCTION(), {
    ...
    buildPrecheck(options) {
        if (this.loc)
            return Quest.IO.falsemsg(Quest.lang.construction_already, options);
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (options.char.loc !== 'beach')
            return Quest.IO.falsemsg('Mandy laments that she is not on a sandy beach, and is therefore unable build a sandcastle.');
        return true;
    },
