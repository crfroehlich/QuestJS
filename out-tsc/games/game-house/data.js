...
Quest.World.createItem('player', Quest.Templates.PLAYER(), {
    ...
    getSharp() {
        ...
        if (Quest.World.w.glass_shard.loc === Quest.World.player.name)
            return Quest.World.w.glass_shard;
        ...
        if (Quest.World.w.crocodile_tooth.loc === Quest.World.player.name)
            return Quest.World.w.crocodile_tooth;
        return null;
    },
    loc: 'highfield_lane',
    parserPriority: -50,
    ...
    receiveItems: [
        {
            f() {
                ...
                Quest.IO.msg("'Give me the boots,' says Mandy.");
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
                Quest.IO.msg('{nv:char:look:true} at her in surprise, then sadness. Forlornly he sits on the floor, and slowly pulls off the boots, before handing them to Mandy. Mandy cannot look at his face without feeling guilty.', options);
            },
    ...
    talkto() {
        if (Quest.World.currentLocation.zone === 'external') {
            ...
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg("'Will I ever escape this scary house?' Mandy asks herself.|'Of course you will!' she replies confidently. 'You just have to believe in yourself!'|'Don't give me that bullshit, you're as scared as I am.'|'Of course I'm scared - it's a scary house. But still sure we can get out of here.'|Mandy smiles uncertainly.");
        }
        return true;
    },
    throwAtPodTries: 0,
...
Quest.World.createItem('mobile_phone', SIZE_CHANGING(), {
    ...
    use() {
        if (this.size === 5) {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg("{once:Mandy looks at her phone. 'Shit.' No charge left. She only charged it last night... No, wait, she had found it on her bedroom floor this morning. 'Shit,' she says again.}{notOnce:Mandy looks at her phone, but it stubbornly refuses to have any change.}");
            return true;
        }
        if (this.size === 4) {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg('Mandy looks at her shrunken phone. Maybe it was a bit optimistic thinking it would now be charged, just because it is so much smaller.');
            return true;
        }
        if (this.size > 5) {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg('Her stupid phone is now too big to use!');
            return false;
        }
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        Quest.IO.msg('Her stupid phone is now too small to use!');
        return false;
    },
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 3.
...
Quest.World.createItem('shakespeare_book', {
    afterCarry() {
        if (this.zone !== Quest.World.currentLocation.zone) {
            ...
            if (Quest.World.w.uniform.wet > 0)
                Quest.World.w.uniform.wet--;
        }
    },
    ...
    examine() {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'lounge' does not exist on type '{}'.
            ...
            this.state = 1;
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
            Quest.IO.msg('Mandy glances at her copy of "Antony and Cleopatra". Wait, this is not the same book! This is {nm:item:a}. What has happened to "Antony and Cleopatra"? Ms Coulter will be furious.', { item: this });
        }
        else {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
    ...
    testTake() {
        return Quest.IO.falsemsg(this.take);
    },
    zone: 'external',
});
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 2.
...
Quest.World.createItem('uniform', {
    ...
    examine() {
        ...
        if (this.wet)
            s += ` It is ${this.wetWords[this.wet]}.`;
        ...
        if (Quest.World.w.Winfield_Malewicz.dead)
            s += " It is splatted with Winfield Malewicz's blood.";
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        Quest.IO.msg(s);
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    remove() {
        if (!this.removeFlag) {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg("Mandy gives you a hard stare. 'It is not that sort of game,' she says.|'Wait, are you breaking the fourth wall?' you ask her.|'Don't get pissy with me. I'm not the one asking a sixteen year old girl to undress. Now, I suggest we just move on, and pretend this never happened, okay?'"),
                this.removeFlag = true;
        }
        else {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg('Mandy just shakes her head, looking very disappointed.');
        }
    },
    scenery: true,
    uniforms: {
    ...
    return {
        examine() {
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'alias' does not exist on type '{ scenery... Remove this comment to see the full error message
                ...
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
                Quest.IO.msg(room[`examine_${this.alias}`]);
            }
            else {
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
                Quest.IO.msg(zones[room.zone][this.alias]);
            }
        },
        isLocatedAt(loc, situation) {
            ...
            if (!room.zone)
                return false;
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            if (Quest.World.w[`${loc}_${this.alias}`])
                return false;
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            if (zones[room.zone][this.alias])
                return true;
            return false;
        },
        parserPriority: -15,
...
Quest.World.createItem('generic_door', GENERIC(), {
    ...
    close() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        Quest.IO.metamsg('You do not need to open or close any any doors in the game. Just give a direction, like NORTH or IN, and Mandy will head that way, negotiating any doors on the way on her own.');
        return false;
    },
    open() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
...
Quest.World.createItem('generic_ceiling', GENERIC(), {
    ...
    touch() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (Quest.World.currentLocation.name !== 'telescope_end')
            return Quest.IO.falsemsg('Mandy tries to reach the ceiling, but it is too high.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
    ...
    examine() {
        ...
        if (!Quest.World.w.generic_panelling.examinedIn) {
            ...
            if (Quest.World.currentLocation !== room) {
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
                Quest.IO.msg('More panelling, just like in {nm:room:the}.', { room });
            }
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'wooden_panel' does not exist on type '{}... Remove this comment to see the full error message
                ...
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
                Quest.IO.msg('Mandy carefully examines the wood panelling again, running her fingers over it. The wood may be course and the contours rather poorly defined, but it is still wonderful! She tries to ignore the hole where the panel is missing...');
            }
        }
    },
    isLocatedAt(loc) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (!Quest.World.w[loc].zone)
            return false;
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
...
Quest.Text.text_processors.hereDesc = function (arr, params) {
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    const room = Quest.World.w[Quest.World.player.loc];
    let s;
...
Quest.World.createItem('generic_window', {
    ...
    close() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.bricked_up)
            return Quest.IO.falsemsg('It is already closed. And bricked up.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    ...
    examine() {
        ...
        if (Quest.World.w.generic_window.roomsmashed) {
            ...
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg('Mandy looks out the window at the countryside; fields, trees, and there is her home, a barn conversion her parents purchased three years ago. But how could that be? No way is her home visible from this house; Highfield Lane twists around far too much for that.');
        }
        else {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
            Quest.IO.msg(`Mandy looks out the window at the countryside; fields, trees, and there is her home, a barn conversion her parents purchased three years ago. But how could that be? This window faces ${windowsface} and her home is to the north.`);
        }
    },
    isLocatedAt(loc) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    open() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.bricked_up)
            return Quest.IO.falsemsg('Mandy looks at the bricked up window. Doubtful that will open now.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    ...
    repair() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (!this.roomsmashed)
            return Quest.IO.falsemsg('The windows are not broken.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    ...
    smash() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (this.bricked_up)
            return Quest.IO.falsemsg('Mandy considered breaking a window again... Of course, smashing a bricked-up window is not something she is likely to be able to do.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        ...
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        Quest.IO.msg('Beyond is only blackness. For one vertiginous moment she stares into the void...');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('glass_shards', {
    ...
    examine() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
        Quest.IO.msg('The shards are the remains of the window. Jagged pieces of glass, some as long as her arm, some almost too small to see. They seem to be reflecting the countryside outside the house somehow...');
    },
    parserPriority: -10,
    ...
    take(options) {
        ...
        if (!options.char.testManipulate(this, 'take'))
            return false;
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('glass_shard', SIZE_CHANGING(), {
    ...
    turn() {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'glass_shard' does not exist on type '{}'... Remove this comment to see the full error message
        if (Quest.World.w.glass_shard.loc !== 'controls')
            return Quest.IO.falsemsg('Mandy turns the shard of glass, hoping it looks more pretty in a better light. It does not.');
...
Quest.World.createItem('paper_funnel', SIZE_CHANGING(), Quest.Templates.CONSTRUCTION(['secret_recipe']), {
    ...
    testConstruction(options) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'secret_recipe' does not exist on type '{... Remove this comment to see the full error message
        if (Quest.World.w.secret_recipe.size > 6)
            return Quest.IO.falsemsg('The card is too stiff to do origami with.');
        return true;
    },
});
