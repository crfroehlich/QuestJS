...
Quest.Settings.settings.afterTurn.push(() => {
    if (Quest.World.player.currentWeatherDisabled)
        return;
...
class Weather {
    constructor(name, data) {
        ...
        for (const key in data)
            this[key] = data[key];
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (weatherTypes[name])
            return Quest.IO.errormsg(`Two weather types called "${name}".`);
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    turn() {
        ...
        if (this.wetness) {
            if (!Quest.World.player.currentWeatherWetness)
                Quest.World.player.currentWeatherWetness = 0;
    ...
    outside(includeVisible) {
        ...
        if (Quest.Settings.settings.weatherReportsAssumeYes && Quest.World.currentLocation.noWeather)
            return false;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'weatherReportsAssumeYes' does not exist ... Remove this comment to see the full error message
        if (!Quest.Settings.settings.weatherReportsAssumeYes && !Quest.World.currentLocation.yesWeather)
            return false;
        if (includeVisible)
            return true;
    ...
    report(s, options) {
        if (!this.outside(true))
            return false;
        if (Quest.World.currentLocation.weatherModifier)
            s = Quest.World.currentLocation.weatherModifier.replace('#', s);
...
new Weather('storm', {
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'chance' does not exist on type '{ buffer... Remove this comment to see the full error message
    ongoing() {
        if (Quest.Random.rndm.chance(10))
            this.report('A flash of lightning illuminates the landscape, and a moment later you hear the crack of thunder.');
    },
    start() {
