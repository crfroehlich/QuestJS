...
const CREW = function (isFemale) {
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'status' does not exist on type '{ canRea... Remove this comment to see the full error message
    res.status = 'okay';
    ...
    res.properNoun = true;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'crewman' does not exist on type '{ canRe... Remove this comment to see the full error message
    res.crewman = true;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'relationship' does not exist on type '{ ... Remove this comment to see the full error message
    res.relationship = 0;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'clothing' does not exist on type '{ canR... Remove this comment to see the full error message
    res.clothing = 2;
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'oxygenUseModifier' does not exist on typ... Remove this comment to see the full error message
    res.oxygenUseModifier = 1;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'oxygenUse' does not exist on type '{ can... Remove this comment to see the full error message
    res.oxygenUse = function () {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'status' does not exist on type '{ canRea... Remove this comment to see the full error message
        if (typeof this.status !== 'number')
            return 0;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'baseOxygeUse' does not exist on type '{ ... Remove this comment to see the full error message
        return this.baseOxygeUse * this.oxygenUseModifier;
    };
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'revive' does not exist on type '{ canRea... Remove this comment to see the full error message
    ...
    res.examine = function (options) {
        ...
        switch (this.clothing) {
            case 0:
                s = ' {pv:char:be:true} naked.';
                break;
            case 1:
                s = ' {pv:char:be:true} in his underwear.';
                break;
            case 2:
                s = ' {pv:char:be:true} wearing a dark grey jumpsuit.';
                break;
        }
    ...
    res.stasis = function () {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'stasisPodCount' does not exist on type '... Remove this comment to see the full error message
        this.stasisPodCount = 0;
    };
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'stasisPod' does not exist on type '{ can... Remove this comment to see the full error message
    ...
    // Probe deployment
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'deployProbeAction' does not exist on typ... Remove this comment to see the full error message
    res.deployProbeAction = 0;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'deployProbeCount' does not exist on type... Remove this comment to see the full error message
    res.deployProbeCount = 0; // number deployed in one batch
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'deployProbeTotal' does not exist on type... Remove this comment to see the full error message
    res.deployProbeTotal = 0; // number deployed in this system
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'deployProbeOverallTotal' does not exist ... Remove this comment to see the full error message
    res.deployProbeOverallTotal = 0; // total
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'deployProbe' does not exist on type '{ c... Remove this comment to see the full error message
    res.deployProbe = function (arr) {
        ...
        switch (this.deployProbeAction) {
            case 0:
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'probeAction0' does not exist on type '{ ... Remove this comment to see the full error message
            ...
            case 2:
                ...
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'actuallyDeployProbe' does not exist on t... Remove this comment to see the full error message
                this.actuallyDeployProbe(count);
                break;
            case 3:
                // ts-error-fixed ts-migrate(2339) FIXME: Property 'probeAction3' does not exist on type '{ ... Remove this comment to see the full error message
    ...
    res.actuallyDeployProbe = function (count) {
        ...
        // housekeeping
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'probesRemaining' does not exist on type ... Remove this comment to see the full error message
...
function planetAnalysis(response) {
    ...
    if (Object.keys(arr).length === 0)
        return Quest.IO.falsemsg('You should talk to Aada or Ostap about that stuff.');
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'Xsansi' does not exist on type '{}'.
...
function trackRelationship(npc, inc, code) {
    if (npc.relationshipTracker === undefined)
        npc.relationshipTracker = '~';
...
function getProbes() {
    ...
    for (const key in Quest.World.w) {
        ...
        if (Quest.World.w[key].clonePrototype === Quest.World.w.probe_prototype)
            arr.push(Quest.World.w[key]);
    }
    return arr;
}
...
// ts-error-fixed ts-migrate(2339) FIXME: Property 'deckNames' does not exist on type '{ per... Remove this comment to see the full error message
Quest.Settings.settings.deckNames = { layer1: 'Deck 2', layer3: 'Deck 1', layer4: 'Deck 3' };
function updateMap() {
    if (!document.querySelector('#layer1'))
        return;
    ...
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!currentDeck)
        return Quest.IO.errormsg(`No deckName for ${Quest.World.player.loc}`);
    ...
    for (const key in Quest.World.w) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (Quest.World.w[key].svgId)
            document.querySelector(`#${Quest.World.w[key].svgId}`).style.fill = isRoomPressured(Quest.World.w[key]) ? '#777' : '#222';
    }
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    const mySvgId = Quest.World.w[Quest.World.player.loc].svgId;
    let otherSvgId;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'Xsansi' does not exist on type '{}'.
    if (Quest.World.w.Xsansi.locate)
        otherSvgId = Quest.World.w[Quest.World.w[Quest.World.w.Xsansi.locate].loc].svgId;
    ...
    if (mySvgId === otherSvgId) {
        ...
        // ts-error-fixed ts-migrate(2531) FIXME: Object is possibly 'null'.
        if (mySvgId)
            document.querySelector(`#${mySvgId}`).style.fill = 'yellow';
        // ts-error-fixed ts-migrate(2531) FIXME: Object is possibly 'null'.
        if (otherSvgId)
            document.querySelector(`#${otherSvgId}`).style.fill = 'blue';
    }
    // ts-error-fixed ts-migrate(2531) FIXME: Object is possibly 'null'.
...
function isRoomPressured(room) {
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (typeof room.vacuum === 'string')
        room = Quest.World.w[room.vacuum];
    return !room.vaccum;
}
