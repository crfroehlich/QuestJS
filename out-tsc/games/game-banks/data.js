...
Quest.World.createItem('me', Quest.Templates.PLAYER(), {
    ...
    testMove(ex) {
        ...
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (typeof room1.vacuum === 'string')
            room1 = Quest.World.w[room1.vacuum];
        if (ex.name === '_')
            return true;
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        let room2 = Quest.World.w[ex.name];
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (typeof room2.vacuum === 'string')
            room2 = Quest.World.w[room2.vacuum];
        if (room1.vacuum === room2.vacuum)
            return true;
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (room2.name === 'space')
            return Quest.IO.falsemsg('The external airlock door cannot be opened while the airlock is pressurised.');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
Quest.World.createItem('your_spacesuit', Quest.Templates.WEARABLE(2, ['body']), {
    ...
    testRemove(char) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (isRoomPressured(Quest.World.w[char.loc]))
            return true;
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
...
// ts-error-fixed ts-migrate(2554) FIXME: Expected 0 arguments, but got 3.
Quest.World.createItem('stasis_pod_interior', Quest.Templates.OPENABLE(true), {
    alias: 'stasis pod',
    close(char) {
...
Quest.World.createItem('probe_prototype', Quest.Templates.COUNTABLE([]), {
    ...
    launch(char) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (!char.probeType)
            return Quest.IO.falsemsg('To launch a probe, see either Aada or Ostap. For a satellite see Kyle.');
