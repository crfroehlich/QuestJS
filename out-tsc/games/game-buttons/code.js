...
Quest.IO.io.getItemHtml = function (item, loc, isSubItem, highlight) {
    const verbList = item.getVerbs(loc);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (verbList === undefined) {
        Quest.IO.errormsg(`No verbs for ${item.name}`);
        console.log(item);
    }
...
Quest.Parser.parser.isContact = function (o) {
    return o.contact;
};
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('HangUp', {
    ...
    script() {
        ...
        if (!Quest.World.player.onPhoneTo)
            return Quest.IO.failedmsg('You are not on a call.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'phone' does not exist on type '{}'.
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('DialogTest', {
    ...
    script(objects) {
        ...
        Quest.IO.io.menuFunctions[funcName]('Pick a colour?', choices, (result) => {
