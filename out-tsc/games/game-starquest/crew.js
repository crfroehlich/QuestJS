...
const CANDIDATE = function (female) {
    ...
    if (!res.entering)
        res.entering = '{nm:char} enters the bridge.';
    ...
    if (!res.leaving)
        res.leaving = '{nm:char} leaves the bridge.';
    return res;
};
const getCrew = function () { };
const getCandidates = function () {
    return Quest.Utilities.scopeBy((o) => o.candidate).sort((a, b) => {
...
Quest.World.createItem('helmsman_go_to_star', Quest.NPC.TOPIC(false), {
    ...
    script() {
        ...
        Quest.IO.showMenuDiag('Which star system?', stars.getStarNames(), (result) => {
            log(result);
