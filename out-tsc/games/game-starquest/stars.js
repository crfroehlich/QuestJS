...
const stars = {
    ...
    draw(name) {
        ...
        if (Quest.World.w.ship.arrivedAtSector)
            svg.push(`<text class="map-text" x="353" y="12" fill="silver" onclick="stars.toggleStarMap(${Quest.World.w.ship.showStarMap})">Toggle</text>`);
    ...
    getSystemOrLocation(isLocation, name) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'ship' does not exist on type '{}'.
        if (name === undefined)
            name = Quest.World.w.ship.Quest.World.currentLocation;
