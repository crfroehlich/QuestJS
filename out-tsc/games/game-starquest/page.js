...
Quest.World.createItem('missions', {
    pageOption: true,
    verbFunction(verbList) {
        verbList.pop();
        for (const m of missions.getList())
            verbList.push(m.alias);
    },
});
...
Quest.World.createItem('crew_roster', {
    ...
    verbFunction(verbList) {
        ...
        if (Quest.World.w.ship.arrivedAtSector) {
            for (const o of roster.getCrew())
                verbList.push(o.alias);
        }
        else {
            for (const o of getCandidates())
                verbList.push(o.alias);
        }
    },
});
    ...
    askDiag(s, fromLink) {
        ...
        for (const key in encyclopedia) {
            if (regex.test(key)) {
                // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 1.
                if (!fromLink)
                    Quest.IO.msg(`On your PAGE you search for "${s}".`);
                const strs = [];
