...
Quest.Text.addDirective('rope', (arr, params) => `<span style="font-family:Montserrat">${arr.join(':')}</span>`);
...
const wrapScript = function (obj1, obj2) {
    ...
    if (obj2.fist_wrapped)
        return Quest.IO.failedmsg('It already is.');
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('ThrowThrough', {
    ...
    script(objects) {
        ...
        if (!dest.isThrowThroughable)
            return Quest.IO.failedmsg("You can't chuck stuff through {nm:dest:the}.", { dest });
        if (!dest.isThrowThroughable(item))
...
const smashWithScript = function (item, dest) {
    ...
    if (item === Quest.World.w.crowbar) {
        ...
        if (Quest.World.w.Professor_Kleinscope.isHere())
            Quest.IO.msg('Strangely, Professor Kleinscope does not seem to notice.');
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'office_window' does not exist on type '{... Remove this comment to see the full error message
