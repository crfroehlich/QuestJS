...
Quest.Utilities.util.openingTimes = function () {
    ...
    if (Quest.Utilities.util.isAfter('1700'))
        return Quest.IO.falsemsg('The business is now closed.');
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'isAfter' does not exist on type '{}'.
    if (!Quest.Utilities.util.isAfter('0800'))
        return Quest.IO.falsemsg('The business closed until eight.');
    return true;
};
...
Quest.Text.addDirective('timeOfDayComment', (arr, params) => {
    ...
    if (!location.timeStatus)
        return '';
    let { hour } = time;
    for (let i = 0; i < location.timeStatus.length; i++) {
        if (hour < location.timeStatus[i].to)
            return location.timeStatus[i].t;
        hour -= location.timeStatus[i].to;
    }
    return 'NONE';
});
// ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
Quest.Text.addDirective('npcStatus', (arr, params) => {
    const result = [];
