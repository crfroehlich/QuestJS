export const cloakHere = function (Quest) {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'cloak' does not exist on type '{}'.
    if (Quest.World.w.cloak.isAtLoc('me'))
        return true;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'cloak' does not exist on type '{}'.
    if (Quest.World.w.cloak.isHere())
        return true;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'cloak' does not exist on type '{}'.
    if (Quest.World.w.cloak.isAtLoc('hook') && Quest.World.player.isAtLoc('cloakroom'))
        return true;
    return false;
};
export const init = (Quest) => {
    ...
    Quest.Text.addDirective('cloakHere', (arr, params) => (cloakHere(Quest) ? arr[0] : arr[1]));
