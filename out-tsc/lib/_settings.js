...
export class Settings {
    constructor(data) {
        ...
        this.playMode = 'game';
        this.roomTemplate = [];
        this.saveLoadExcludedAtts = [];
        this.scriptToLoad = [];
        this.statsData = [];
        this.thanks = [];
        this.themes = [];
        this.walkthroughMenuResponses = [];
        this.warnings = '';
    ...
    loadCssFile(filename, doc = document, path = '') {
    ...
    loadFavicon() {
        if (!this.favicon)
            this.favicon = `${this.iconsFolder}favicon.png`;
        const link = document.createElement('link');
    ...
    loadScript(filename, doc = document) {
        if (doc) {
            this.scriptDoc = doc;
        }
    ...
    scriptOnLoad() {
        if (this.scriptLoading && this.scriptLoadLogging)
            console.log(`Loaded ${this.scriptLoading}`);
        if (!this.scriptToLoad || this.scriptToLoad.length === 0) {
            if (this.scriptLoadLogging)
        ...
        if (this.scriptLoadLogging)
            console.log(`Loading ${this.scriptLoading}`);
    ...
    setUpDialog() {
        ...
        if (!diag)
            return;
        const diagTitle = document.querySelector('#dialog-title');
        if (!diagTitle)
            return;
        ...
        const diagButton = document.querySelector('#dialog-button');
        if (!diagButton)
            return;
