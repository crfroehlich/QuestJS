...
export const parser = {
    ...
    findInScope(s, scopes, cmdParams) {
    ...
    isContained(item) {
        ...
        for (const container of containers) {
            if (container.closed)
                continue;
            if (item.isAtLoc(container.name, Quest.World.world.PARSER))
                return true;
        }
        return false;
    },
    ...
    isHeldByNpc(item) {
        ...
        for (const npc of npcs) {
            if (item.isAtLoc(npc.name, Quest.World.world.PARSER))
                return true;
        }
        return false;
    },
    ...
    isHereOrLocationContained(item) {
        if (parser.isHere(item))
            return true;
        if (parser.isLocationContained(item))
            return true;
        return false;
    },
    ...
    isLocationContained(item) {
        ...
        for (const container of containers) {
            if (container.closed)
                continue;
            if (container.isUltimatelyHeldBy(Quest.World.player))
                continue;
            if (item.isAtLoc(container.name, Quest.World.world.PARSER))
                return true;
        }
    ...
    isUnconstructed(item) {
        if (!item.loc && item.construction)
            return true;
        return false;
    },
    ...
    matchToName(s, scopes, cmdParams, objectWordList) {
    ...
    parseSingle(inputText) {
        ...
        for (let i = 0; i < parser.currentCommand.tmp.objects.length; i++) {
            if (!Array.isArray(parser.currentCommand.tmp.objects[i]))
                continue;
            for (let j = 0; j < parser.currentCommand.tmp.objects[i].length; j++) {
                if (parser.currentCommand.tmp.objects[i][j] instanceof Array) {
                    if (parser.currentCommand.tmp.objects[i][j].length === 1) {
                        ...
                        fn(Quest.lang.disambig_msg, parser.currentCommand.tmp.objects[i][j], (result) => {
    ...
    scoreObjectMatch(s, item, cmdParams) {
        ...
        else if (item.synonyms && item.synonyms.some((el) => el.startsWith(s))) {
    ...
    specialText: {
        fluid: {
            error(text) {
                if (Quest.Settings.settings.fluids.includes(text))
                    return false;
        ...
        number: {
            error(text) {
                if (text.match(/^\d+$/))
                    return false;
                if (Quest.lang.numberUnits.includes(text))
                    return false;
                return true;
            },
            exec(text) {
