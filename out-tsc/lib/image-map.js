...
map.redraw = function (offX, offY) {
    ...
    if (Quest.World.w[Quest.World.player.loc].mapX)
        Quest.World.player.mapX = Quest.World.w[Quest.World.player.loc].mapX / Quest.Settings.settings.mapScale;
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (Quest.World.w[Quest.World.player.loc].mapY)
        Quest.World.player.mapY = Quest.World.w[Quest.World.player.loc].mapY / Quest.Settings.settings.mapScale;
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (Quest.World.w[Quest.World.player.loc].mapRegion)
        Quest.World.player.mapRegion = Quest.World.w[Quest.World.player.loc].mapRegion;
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'mapImages' does not exist on type '{ per... Remove this comment to see the full error message
    const mapImage = Quest.Settings.settings.mapImages.find((el) => el.name === Quest.World.player.mapRegion);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!mapImage)
        return Quest.IO.errormsg(`Failed to find a map region called '${Quest.World.player.mapRegion}'`);
    const result = [];
    ...
    for (const point of Quest.Settings.settings.mapPointsOfInterest) {
        ...
        if (point.mapRegion !== Quest.World.player.mapRegion)
            continue;
