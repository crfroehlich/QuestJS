...
export const DEFAULT_OBJECT = {
    ...
    isApparentTo(situation) {
        ...
        if (situation === Quest.World.world.LOOK && this.scenery)
            return false;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'scenery' does not exist on type '{ prono... Remove this comment to see the full error message
        if (situation === Quest.World.world.SIDE_PANE && this.scenery && !Quest.Settings.settings.showSceneryInSidePanes)
            return false;
        ...
        if (situation === Quest.World.world.SIDE_PANE && this.player)
            return false;
        return true;
    },
    isAtLoc(loc, situation) {
        ...
        if (!Quest.World.w[loc])
            Quest.IO.errormsg(`The location name \`${loc}\`, does not match anything in the game.`);
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 1 arguments, but got 2.
        if (!this.isLocatedAt(loc, situation))
            return false;
    ...
    moveToFrom(options, toLoc, fromLoc) {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'loc' does not exist on type '{ pronouns:... Remove this comment to see the full error message
        if (options.fromLoc === undefined)
            options.fromLoc = this.loc;
        if (options.fromLoc === options.toLoc)
            return;
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (!Quest.World.w[options.fromLoc])
            Quest.IO.errormsg(`The location name \`${options.fromLoc}\`, does not match anything in the game.`);
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (!Quest.World.w[options.toLoc])
            Quest.IO.errormsg(`The location name \`${options.toLoc}\`, does not match anything in the game.`);
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'loc' does not exist on type '{ pronouns:... Remove this comment to see the full error message
        ...
        if (options.toLoc === Quest.World.player.name && this.afterTake !== undefined)
    ...
    saveLoadExclude(att) {
        ...
        if (typeof this[att] === 'object' && !Array.isArray(this[att]))
            return true;
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (this[att] instanceof Quest.World.Exit)
            return true;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hasMatch' does not exist on type '{}'.
        if (Quest.Utilities.array.hasMatch(Quest.Settings.settings.saveLoadExcludedAtts, att))
            return true;
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hasMatch' does not exist on type '{}'.
    ...
    scopeSnapshot(mode) {
        ...
        if (this.getContents) {
            ...
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            if (!this[`can${mode}ThroughThis`]() && !this.scopeStatus[`room${mode}`] && this !== Quest.World.player)
                return;
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'getContents' does not exist on type '{ p... Remove this comment to see the full error message
...
export const DEFAULT_ROOM = {
    ...
    getExitObjs(options) {
        if (options === undefined)
            options = {};
        const list = [];
        if (options.excludeAlsoDir === undefined)
    ...
    goItem(obj, dir, char) {
        ...
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (!obj[att])
            return Quest.IO.failedmsg(lang[`cannot_go_${dir.toLowerCase()}`], { char, item: obj });
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (!this[obj[att]])
            return Quest.IO.errormsg(`Trying to 'go ${dir.toLowerCase()}' using unknown exit '${obj[att]}' for ${this.name}`);
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
