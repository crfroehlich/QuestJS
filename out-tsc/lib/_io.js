...
function _msg(s, params, options) {
    ...
    for (let line of processed.split('|')) {
        ...
        if (settings.convertDoubleDash && !Quest.Utilities.test.testing)
...
export function errormsg(s, suppressTrace) {
    ...
    if (Quest.Utilities.test.errorOutput !== undefined) {
        // This is an expected error in a unit test
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'errorOutput' does not exist on type '{}'... Remove this comment to see the full error message
        Quest.Utilities.test.errorOutput.push(s);
        return false;
    }
...
export function wait(delay, text, func) {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'testing' does not exist on type '{}'.
    if (Quest.Utilities.test.testing || settings.walkthroughInProgress)
        return;
    if (delay === undefined) {
        io.addToOutputQueue({
...
export function showMenu(title, options, fn) {
    ...
    io.input(title, options, false, fn, (options) => {
...
function showMenuNumbersOnly(title, options, fn) {
    ...
    Quest.Parser.parser.overrideWith((s) => {
        io.menuResponse(s);
    });
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'input' does not exist on type '{ nextid:... Remove this comment to see the full error message
    io.input(title, options, disableTextFunction, fn, (options) => {
        for (let i = 0; i < options.length; i++) {
    ...
    const disableTextFunction = function (disable) {
        if (disable) {
            ...
            if (!Quest.Utilities.test.testing) {
                ...
                if (el)
                    el.remove(); // may not exist in walk-through
            }
        }
    };
...
export function askText(title, fn) {
    ...
    io.disable(2);
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'overrideWith' does not exist on type '{}... Remove this comment to see the full error message
    Quest.Parser.parser.overrideWith((result) => {
...
export const io = {
    addClassForClass(oldClass, newClass) {
    ...
    appendItem(item, htmlDiv, loc, isSubItem, highlight) {
    ...
    enable(level) {
        ...
        if (!level)
            level = 1;
        if (!io.disableLevel || level > io.disableLevel)
            return;
        io.disableLevel = 0;
        // ts-error-fixed ts-migrate(2531) FIXME: Object is possibly 'null'.
    ...
    getItemHtml(item, loc, isSubItem, highlight) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (typeof item.getVerbs !== 'function')
            return errormsg(`Item with bad getVerbs: ${item.name}`);
    ...
    getToolbarHTML(data = {}) {
        ...
        if (data.room)
            return Quest.Utilities.sentenceCase(Quest.lang.getName(Quest.World.w[Quest.World.player.loc], { article: Quest.Utilities.DEFINITE }));
    ...
    input(title, options, disableTextFunction, reactFunction, displayFunction, failFunction) {
        ...
        io.disableTextFunction = disableTextFunction || function (disable) {
            if (disable)
                io.disable(3);
            if (!disable)
                io.enable();
    ...
    menuResponse(n) {
        ...
        for (let i = io.menuStartId; i < io.nextid; i++)
            document.querySelector(`#n${i}`).remove();
        // handle bad number
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'menuOptions' does not exist on type '{ n... Remove this comment to see the full error message
    ...
    msgInputText(s) {
        ...
        if (Quest.SaveLoad.saveLoad.transcript)
            Quest.SaveLoad.saveLoad.transcriptWalkthrough.push(`    "${s}",`);
    ...
    outputFromQueue() {
        ...
        if (data?.action === 'wait') {
            ...
            if (!data.text)
                data.text = Quest.lang.click_to_continue;
            io.print(data);
        }
    ...
    print(data) {
        ...
        if (data.html) {
            ...
            for (const s of io.allowedHtmlAttrs)
                if (data[s])
                    html += ` ${s}="${data[s]}"`;
    ...
    showInTab(html, title = 'Quest JS Tab') {
    ...
    speak(str, altVoice) {
        if (!io.spoken)
            return;
        if (!io.voice) {
            io.voice = io.synth.getVoices().find((el) => /UK/.test(el.name) && /Female/.test(el.name));
            if (!io.voice)
        ...
        if (!io.voice2) {
            ...
            io.voice2 = io.synth.getVoices().find((el) => /UK/.test(el.name) && /Male/.test(el.name));
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'voice2' does not exist on type '{ nextid... Remove this comment to see the full error message
            if (!io.voice2)
