...
export const saveLoad = {
    ...
    dirGame() {
        ...
        if (!Quest.Settings.settings.saveComment)
            arr0.pop();
        let s = arr0.join('');
        ...
        if (Array.isArray(value)) {
            try {
                ...
                if (typeof value[0] === 'number')
                    return `${key}:numberarray:${Quest.SaveLoad.saveLoad.encodeNumberArray(value)};`;
                return '';
            }
            catch (error) {
    ...
    loadTheWorld(s, removeHeader) {
        ...
        for (const key in Quest.World.w) {
            if (Quest.World.w[key].clonePrototype)
                delete Quest.World.w[key];
        }
    ...
    setFromArray(obj, arr) {
        ...
        for (const el of keys)
            delete obj[el];
        for (const el of arr)
            Quest.SaveLoad.saveLoad.decode(obj, el);
    },
    // ts-error-fixed ts-migrate(7006) FIXME: Parameter 's' implicitly has an 'any' type.
    setLoadString(s) {
        const parts = s.split('=');
...
//# sourceMappingURL=_saveload.js.map