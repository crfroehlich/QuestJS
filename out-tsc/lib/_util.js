...
export function formatList(itemArray, options) {
    ...
    if (Quest.Settings.settings.oxfordComma && l.length === 2 && options.lastJoiner)
        return `${l[0]} ${options.lastJoiner} ${l[1]}`;
    do {
        s += l.shift();
        if (l.length === 1 && options.lastJoiner) {
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'oxfordComma' does not exist on type '{ p... Remove this comment to see the full error message
            if (Quest.Settings.settings.oxfordComma)
                s += options.sep;
...
export const array = {
    ...
    filterByAttribute(ary, attName, value) {
        return ary.filter((el) => el[attName] === value);
    },
    fromTokens(ary, scope, cmdParams) {
        const items = [];
        while (ary.length > 0) {
    ...
    // If el is present more than once, it goes with the first.
    // If el is the last element, and circular is true it return the fist element and false otherwise.
    nextFlagged(ary, el, att, circular) {
        let o = el;
    ...
    oneFromTokens(ary, scope, cmdParams = {}) {
...
export const util = {
    addChangeListener(object, attName, func, test = util.defaultChangeListenerTest) {
    ...
    changePOV(char, pronouns) {
        if (typeof char === 'string') {
            ...
            if (!Quest.World.w[char])
                return Quest.IO.errormsg(`Failed to change POV, no object called '${char}'`);
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    defaultChangeListenerTest(object, currentValue, oldValue, attName) {
        return currentValue !== oldValue;
    },
    defaultExitUse(char, exit) {
        if (!exit)
            exit = this;
        if (char.testMove && !char.testMove(exit))
            return false;
        if (exit.isLocked()) {
            return Quest.IO.falsemsg(exit.lockedmsg ? exit.lockedmsg : Quest.lang.locked_exit, { char, exit });
        }
        if (exit.testExit && !exit.testExit(char, exit))
            return false;
        for (const el of char.getCarrying()) {
            if (el.testCarry && !el.testCarry({ char, exit, item: el }))
                return false;
        }
    ...
    defaultSimpleExitUse(char, exit) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (exit.name === '_')
            return Quest.IO.errormsg(`Trying to move character to location "_" from room ${exit.origin.name}. This is probably a bug, as "_" is used to flag a destination that cannot be reached.`);
        if (exit === undefined)
            exit = this;
    ...
    findResponse(params, list) {
        for (const item of list) {
            if (item.test && !item.test(params))
                continue;
            if (item.responses)
                return util.findResponse(params, item.responses);
            return item;
        }
        return false;
    },
    ...
    findTopic(alias, char, n = 1) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (Quest.World.w[alias])
            return Quest.World.w[alias];
        for (const key in Quest.World.w) {
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            const o = Quest.World.w[key];
    ...
    getNameModifiers(item, options) {
        ...
        if (item.nameModifierFunction)
            item.nameModifierFunction(list, options);
        if (list.length === 0)
            return '';
    ...
    getObj(name) {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (!name)
            return Quest.IO.errormsg(`Trying to find an object in util.getObj, but name is ${name}`);
    ...
    getStdDateTimeDict(options) {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'hour' does not exist on type '{}'.
        dict.hour = time.getHours();
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'date' does not exist on type '{}'.
        dict.date = time.getDate();
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'weekday' does not exist on type '{}'.
        dict.weekday = time.toLocaleString('default', { weekday: 'long' });
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'month' does not exist on type '{}'.
        dict.month = time.toLocaleString('default', { month: 'long' });
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'year' does not exist on type '{}'.
    ...
    multiIsUltimatelyHeldBy(obj, objNames) {
    ...
    registerTimerEvent(eventName, triggerTime, interval) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        if (!Quest.Settings.settings.eventFunctions[eventName])
            Quest.IO.errormsg(`A timer is trying to call event '${eventName}' but no such function is registered.`);
    ...
    registerTimerFunction(eventName, func) {
        if (Quest.World.world.isCreated && !Quest.Settings.settings.saveDisabled) {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    ...
    seconds(seconds, minutes = 0, hours = 0, days = 0) {
        ...
        if (Quest.Settings.settings.dateTime.convertSeconds)
            return Quest.Settings.settings.dateTime.convertSeconds(seconds, minutes, hours, days);
        return ((((days * 24) + hours) * 60) + minutes) * 60 + seconds;
    },
    setChangeListenersLoadString(s) {
        ...
        if (parts[0] !== 'ChangeListenersUsedStrings')
            return Quest.IO.errormsg('Expected ChangeListenersUsedStrings to be first');
    ...
    testForRecursion(char, item) {
        ...
        while (Quest.World.w[contName]) {
            ...
            if (Quest.World.w[contName].loc === item.name)
                return Quest.IO.falsemsg(Quest.lang.container_recursion, { char, container: this, item });
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
...
export function respond(params, list, func) {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'responseDebug' does not exist on type '{... Remove this comment to see the full error message
    if (Quest.Settings.settings.responseDebug)
        log(params);
    ...
    // console.log(params)
    if (response.script)
        response.script.bind(params.char)(params);
    if (response.msg) {
        if (params.char) {
