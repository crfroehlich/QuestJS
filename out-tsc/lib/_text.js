...
export const setLoadString = function (s) {
    const parts = s.split('=');
    ...
    if (parts[0] !== 'TPUsedStrings')
        return Quest.IO.errormsg('Expected TP to be first');
...
// and return a string.
export const addDirective = function (name, fn) {
...
const processTextTp = function (str, params) {
...
// we get nested.
const findFirstToken = function (s) {
    const end = s.indexOf('}');
    if (end === -1) {
        return false;
    }
...
const i = function (arr, params) {
    return `<i>${arr.join(':')}</i>`;
};
const b = function (arr, params) {
    return `<b>${arr.join(':')}</b>`;
};
const u = function (arr, params) {
    return `<u>${arr.join(':')}</u>`;
};
const s = function (arr, params) {
    return `<strike>${arr.join(':')}</strike>`;
};
const code = function (arr, params) {
    return `<code>${arr.join(':')}</code>`;
};
const sup = function (arr, params) {
    return `<sup>${arr.join(':')}</sup>`;
};
const sub = function (arr, params) {
    return `<sub>${arr.join(':')}</sub>`;
};
const huge = function (arr, params) {
    return `<span style="font-size:2em">${arr.join(':')}</span>`;
};
const big = function (arr, params) {
    return `<span style="font-size:1.5em">${arr.join(':')}</span>`;
};
const small = function (arr, params) {
    return `<span style="font-size:0.8em">${arr.join(':')}</span>`;
};
const tiny = function (arr, params) {
    return `<span style="font-size:0.6em">${arr.join(':')}</span>`;
};
const smallcaps = function (arr, params) {
    return `<span style="font-variant:small-caps">${arr.join(':')}</span>`;
};
const cap = function (arr, params) {
    return Quest.Utilities.sentenceCase(arr.join(':'));
};
const title = function (arr, params) {
    return Quest.Utilities.titleCase(arr.join(':'));
};
const upper = function (arr, params) {
    return arr.join(':').toUpperCase();
};
const lower = function (arr, params) {
    return arr.join(':').toLowerCase();
};
const rainbow = function (arr, params) {
    const s = arr.pop();
...
const _charSwap = function (c, upper, lower) {
    if (c.match(/[A-Z]/))
...
const encode = function (arr, params) {
...
const blur = function (arr, params) {
    const n = arr.shift();
    return `<span style="color:transparent;text-shadow: 0 0 ${n}px rgba(0,0,0,1);">${arr.join(':')}</span>`;
};
const font = function (arr, params) {
    const f = arr.shift();
    return `<span style="font-family:${f}">${arr.join(':')}</span>`;
};
const klass = function (arr, params) {
    const c = arr.shift();
    return `<span class="${c}">${arr.join(':')}</span>`;
};
const colour = function (arr, params) {
    const c = arr.shift();
    return `<span style="color:${c}">${arr.join(':')}</span>`;
};
const color = colour;
const back = function (arr, params) {
    const c = arr.shift();
    return `<span style="background-color:${c}">${arr.join(':')}</span>`;
};
const dialogue = function (arr, params) {
    let prefix = '<span';
    const style = arr.shift();
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (arr.length < 1)
        Quest.IO.errormsg(`Failed to find enough parts in text processor 'dialog' (${params.tpOriginalString})`);
    if (style.startsWith('.')) {
        ...
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (arr.length < 2)
            Quest.IO.errormsg(`Failed to find enough parts in text processor 'dialog' without a class (${params.tpOriginalString})`);
...
const random = function (arr, params) {
    return arr[Math.floor(Math.random() * arr.length)];
};
const selectNone = function (arr, params) {
    return select(arr, params, 'none');
};
const selectWrap = function (arr, params) {
    return select(arr, params, 'wrap');
};
const selectStart = function (arr, params) {
    return select(arr, params, 'start');
};
const selectEnd = function (arr, params) {
    return select(arr, params, 'end');
};
const select = function (arr, params, opt) {
    let name = arr.shift();
    if (name.match(/\./)) {
    ...
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!o)
        Quest.IO.errormsg(`Failed to find an object called "${name}" in text processor select.`);
    const l = o[arr[0]];
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (l === undefined)
        Quest.IO.errormsg(`Failed to find an attribute called "${arr[0]}" for "${name}" in text processor "select" directive.`);
    if (Array.isArray(l)) {
        const n = o[arr[1]];
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (!l)
            Quest.IO.errormsg(`Failed to find a secondary attribute called "${arr[1]}" for "${name}" in text processor "select" directive.`);
...
const _findObject = function (name, params, arr) {
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (params && params[name])
        return typeof params[name] === 'string' ? Quest.World.w[params[name]] : params[name];
    if (name === 'player')
        return Quest.World.player;
    if (name === 'Quest.World.currentLocation')
        return Quest.World.currentLocation;
    if (name === 'settings')
        return Quest.Settings.settings;
    if (name === 'params')
        return params;
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (Quest.World.w[name])
        return Quest.World.w[name];
    const ary = name.split('.');
    if (ary.length === 1)
        return undefined;
    if (ary.length > 2) {
...
const multi = function (arr, params) {
    if (!params.multiple)
        return '';
    return `${Quest.Utilities.sentenceCase(params.item.alias)}: `;
};
const show = function (arr, params) {
    const name = arr.shift();
    if (params[name] !== undefined) {
        if (typeof params[name] === 'object')
            return getWhatever(params[name][arr[0]], params, params[name]);
    ...
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'show' (${params.tpOriginalString})`);
    return getWhatever(obj[arr[0]], params, obj);
};
const object = show;
const getWhatever = function (val, params, obj) {
    if (val === false)
        return Quest.lang.tp_false;
    if (val === true)
        return Quest.lang.tp_true;
    if (val === undefined)
        return '';
    ...
    if (typeof val !== 'function')
        return Quest.IO.errormsg(`Got a value of a type I was not expecting in show: ${typeof val}`);
    const func = val.bind(obj);
    return func(params);
};
const contents = function (arr, params) {
    const name = arr.shift();
    const obj = typeof params[name] === 'object' ? params[name] : _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'contents' (${params.tpOriginalString})`);
...
const rndalt = function (arr, params) {
    const name = arr.shift();
    if (params[name]) {
...
const number = function (arr, params) {
    const name = arr.shift();
    if (name.match(/^\d+$/))
        return Quest.lang.toWords(parseInt(name));
    if (typeof params[name] === 'number')
        return Quest.lang.toWords(params[name]);
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'number' (${params.tpOriginalString})`);
    if (typeof obj[arr[0]] === 'number')
        return Quest.lang.toWords(obj[arr[0]]);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    return Quest.IO.errormsg(`Failed to find a number for object '${name}' in text processor (${params.tpOriginalString})`);
};
const ordinal = function (arr, params) {
    const name = arr.shift();
    if (name.match(/^\d+$/))
        return Quest.lang.toOrdinal(parseInt(name));
    if (typeof params[name] === 'number')
        return Quest.lang.toOrdinal(params[name]);
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'number' (${params.tpOriginalString})`);
    if (typeof obj[arr[0]] === 'number')
        return Quest.lang.toOrdinal(obj[arr[0]]);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    return Quest.IO.errormsg(`Failed to find a number for object '${name}' in text processor (${params.tpOriginalString})`);
};
const money = function (arr, params) {
    const name = arr.shift();
    if (name.match(/^\d+$/))
        return Quest.Utilities.displayMoney(parseInt(name));
    if (typeof params[name] === 'number')
        return Quest.Utilities.displayMoney(params[name]);
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'money' (${params.tpOriginalString})`);
...
const dateTime = function (arr, params) {
...
const transitDest = function (arr, params) {
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    const transit = arr[0] ? Quest.World.w[arr[0]] : Quest.World.w[Quest.World.player.loc];
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!transit.transitDoorDir)
        return Quest.IO.errormsg(`Trying to use the 'transitDest' text process directive when the player is not in a transit location (${params.tpOriginalString}).`);
...
const exit = command;
const page = command;
const hour = function (arr, params) {
...
const link = function (arr, params) {
    const s1 = arr.shift();
    const s2 = arr.join(':');
    return `<a href=\"${s2}\" target="_blank">${s1}</a>`;
};
const popup = function (arr, params) {
    const s1 = arr.shift();
    const s2 = arr.join(':');
...
const roomSet = function (arr, params) {
    const n = Quest.World.currentLocation.roomSetOrder - 1;
    return n < arr.length ? arr[n] : '';
};
// -----------  CONDITIONALS  --------------------
const iif = function (arr, params) {
    return handleIf(arr, params, false);
};
const ifNot = function (arr, params) {
    return handleIf(arr, params, true);
};
const handleIf = function (arr, params, reverse) {
    let name = arr.shift();
    let flag;
    if (typeof params[name] === 'boolean')
        return (params[name] ? arr[0] : (arr[1] ? arr[1] : ''));
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'if/ifNot' (${params.tpOriginalString})`);
    name = arr.shift();
    ...
    if (typeof attValue === 'boolean') {
        ...
        if (typeof attValue === 'number') {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            if (isNaN(value))
                return Quest.IO.errormsg(`Trying to compare a numeric attribute, '${name}' with a string (${params.tpOriginalString}).`);
...
const ifIs = function (arr, params) {
    return handleIfIs(arr, params, false);
};
const ifNotIs = function (arr, params) {
    return handleIfIs(arr, params, true);
};
const handleIfIs = function (arr, params, reverse) {
    let name = arr.shift();
    let flag;
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
...
const ifExists = function (arr, params) {
    return handleIfExists(arr, params, false);
};
const ifNotExists = function (arr, params) {
    return handleIfExists(arr, params, true);
};
const handleIfExists = function (arr, params, reverse) {
    let name = arr.shift();
    let flag;
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'if/ifNotExists' (${params.tpOriginalString})`);
    name = arr.shift();
...
const ifLessThan = function (arr, params) {
    return handleIfLessMoreThan(arr, params, false, false);
};
const ifMoreThan = function (arr, params) {
    return handleIfLessMoreThan(arr, params, true, false);
};
const ifLessThanOrEqual = function (arr, params) {
    return handleIfLessMoreThan(arr, params, false, true);
};
const ifMoreThanOrEqual = function (arr, params) {
    return handleIfLessMoreThan(arr, params, true, true);
};
const handleIfLessMoreThan = function (arr, params, moreThan, orEqual) {
    let name = arr.shift();
    let flag;
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'ifLessMoreThan' (${params.tpOriginalString})`);
    name = arr.shift();
    const attValue = typeof obj[name] === 'function' ? obj[name](params) : obj[name];
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (typeof attValue !== 'number')
        return Quest.IO.errormsg(`Trying to use ifLessThan with a non-numeric (or nonexistent) attribute, '${name}' (${params.tpOriginalString}).`);
    let value = arr.shift();
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (isNaN(value))
        return Quest.IO.errormsg(`Trying to compare a numeric attribute, '${name}' with a string (${params.tpOriginalString}).`);
...
const ifHere = function (arr, params) {
    return handleIfHere(arr, params, false, 'loc');
};
const ifNotHere = function (arr, params) {
    return handleIfHere(arr, params, true, 'loc');
};
const ifHeld = function (arr, params) {
    return handleIfHere(arr, params, false, 'name');
};
const ifNotHeld = function (arr, params) {
    return handleIfHere(arr, params, true, 'name');
};
const handleIfHere = function (arr, params, reverse, locAtt) {
    const name = arr.shift();
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'ifHere' (${params.tpOriginalString})`);
    let flag = obj.isAtLoc(Quest.World.player[locAtt], Quest.World.world.ALL);
    if (reverse)
        flag = !flag;
    return (flag ? arr[0] : (arr[1] ? arr[1] : ''));
};
const ifPlayer = function (arr, params) {
    return handleIfPlayer(arr, params, false);
};
const ifNotPlayer = function (arr, params) {
    return handleIfPlayer(arr, params, true);
};
const handleIfPlayer = function (arr, params, reverse) {
    const name = arr.shift();
    let flag;
    const obj = _findObject(name, params, arr);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!obj)
        return Quest.IO.errormsg(`Failed to find object '${name}' in text processor 'if/ifNotPlayer' (${params.tpOriginalString})`);
...
const terse = function (arr, params) {
...
const hereDesc = function (arr, params) {
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
...
const exits = function (arr, params) {
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    const list = Quest.World.w[Quest.World.player.loc].getExitDirs();
...
const nm = function (arr, params) {
    return nameFunction(arr, params, false);
};
const nms = function (arr, params) {
    return nameFunction(arr, params, true);
};
const nameFunction = function (arr, params, isPossessive) {
    const name = arr.shift();
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'article' does not exist on type '{ posse... Remove this comment to see the full error message
    if (arr[0] === 'a')
        options.article = Quest.Utilities.INDEFINITE;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'article' does not exist on type '{ posse... Remove this comment to see the full error message
    if (arr[0] === 'count')
        options.article = Quest.Utilities.COUNT;
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (params[`${subject.name}_count`])
        options[`${subject.name}_count`] = params[`${subject.name}_count`];
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (params[`${name}_count`])
        options[`${subject.name}_count`] = params[`${name}_count`];
    let n = 2;
    while (arr[n]) {
        // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
...
const handlePronouns = function (arr, params, pronoun) {
    const name = arr.shift();
    // ts-error-fixed ts-migrate(2339) FIXME: Property '_findObject' does not exist on type '{ t... Remove this comment to see the full error message
    const subject = _findObject(name, params, arr);
    if (!subject)
...
const pa = function (arr, params) {
    return handlePronouns(arr, params, 'poss_adj');
};
const ob = function (arr, params) {
