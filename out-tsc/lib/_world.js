...
export function createItem(...params) {
    ...
    return createItemOrRoom(params, Quest.Defaults.DEFAULT_ITEM, Quest.Settings.settings.itemCreateFunc);
}
// @DOC
...
export function createRoom(...params) {
    ...
    const o = createItemOrRoom(params, Quest.Defaults.DEFAULT_ROOM, Quest.Settings.settings.roomCreateFunc);
...
export function cloneObject(item, loc, newName) {
    ...
    clone.name = newName === undefined ? Quest.Utilities.util.findUniqueName(item.name) : newName;
...
function createObject(name, listOfHashes) {
    ...
    if (world.isCreated && !Quest.Settings.settings.saveDisabled)
        return Quest.IO.errormsg(`Attempting to use createObject with \`${name}\` after set up. To ensure games save properly you should use cloneObject to create ites during play.`);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (/\W/.test(name))
        return Quest.IO.errormsg(`Attempting to use the prohibited name \`${name}\`; a name can only include letters and digits - no Quest.Utilities.spaces or accented characters. Use the 'alias' attribute to give an item a name with other characters.`);
    // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    if (w[name])
        return Quest.IO.errormsg(`Attempting to use the name \`${name}\` when there is already an item with that name in the world.`);
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (typeof listOfHashes.unshift !== 'function')
        return Quest.IO.errormsg(`The list of hashes for \`${name}\` is not what I was expecting. Maybe you meant to use createItem or createRoom?`);
    ...
    item.verbFunctions = [function (o, verbList) {
            ...
            if (o.use !== undefined)
                verbList.push(Quest.lang.verbs.use);
        }];
    ...
    for (const hash of listOfHashes) {
        if (hash.afterCreation)
            hash.afterCreation(item);
    }
    w[name] = item;
    return item;
}
...
export const world = {
    ...
    begin() {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'intro' does not exist on type '{ perform... Remove this comment to see the full error message
            ...
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'intro' does not exist on type '{ perform... Remove this comment to see the full error message
            for (const el of Quest.Settings.settings.intro)
                Quest.IO.msg(el);
        }
    ...
    endTurn(result) {
        ...
        if (result === world.SUCCESS || (Quest.Settings.settings.failCountsAsTurn && result === world.FAILED)) {
            game.turnCount++;
            game.elapsedTime += Quest.Settings.settings.dateTime.secondsPerTurn;
            for (const key in w)
                w[key].endTurn();
            Quest.Utilities.util.handleChangeListeners();
            for (const el of Quest.Settings.settings.afterTurn)
                el(true);
            world.resetPauses();
            world.update();
            world.saveGameState();
    ...
    enterRoom(exit) {
        ...
        if (currentLocation.visited === 0) {
            if (currentLocation.roomSet) {
                ...
                // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
                for (const el of Quest.Settings.settings.roomSetList[currentLocation.roomSet]) {
                    if (el.visited)
                        currentLocation.roomSetOrder++;
                    if (el.name === currentLocation.name)
                        el.visited = true;
                }
            }
    ...
    enterRoomAfterScripts(exit) {
        ...
        for (const key in currentLocation.afterEnterIf) {
            ...
            if (currentLocation.afterEnterIfFlags.split(' ').includes(key))
                continue;
    ...
    gameTimer() {
        ...
        for (let i = 0; i < game.timerEventNames.length; i++) {
            if (game.timerEventTriggerTimes[i] && game.timerEventTriggerTimes[i] < game.elapsedRealTime) {
                ...
                const flag = Quest.Settings.settings.eventFunctions[game.timerEventNames[i]]();
    ...
    init() {
        ...
        game.ticker = setInterval(world.gameTimer, Quest.Settings.settings.timerInterval);
    ...
    initItem(item) {
        if (Quest.Settings.settings.playMode === 'dev' && item.loc && !w[item.loc]) {
        ...
        if (Quest.Settings.settings.playMode === 'dev' && item.consultable && !Quest.Settings.settings.noAskTell) {
        ...
        if (item.convTopics) {
            item.convTopics.forEach((value, i) => {
                value.loc = item.name;
    ...
    update() {
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (!player)
            return Quest.IO.errormsg('No player object found. This will not go well...');
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        if (player.loc === player.name)
            return Quest.IO.errormsg('The location assigned to the player is the player itself.');
...
export function Exit(name, hash) {
    ...
    this.isGuarded = function () {
        ...
        for (const s of this.guardedBy) {
            // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
            const guard = w[s];
            if (guard.isGuarding && guard.isGuarding(this))
                guards.push(guard);
        }
...
Quest.World = World;
//# sourceMappingURL=_world.js.map