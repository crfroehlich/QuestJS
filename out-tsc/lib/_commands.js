...
export const commands = [
    ...
    new Quest.Command.Cmd('MetaRestart', {
        script() {
            Quest.IO.askText(Quest.lang.restart_are_you_sure, (result) => {
    ...
    new Quest.Command.Cmd('Sit', {
        ...
        script() {
            const objs = Quest.Utilities.scopeBy((el) => el.siton && el.isAtLoc(Quest.World.player.loc));
            console.log(objs);
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            if (objs.length === 0)
    ...
    new Quest.Command.Cmd('Recline', {
        ...
        rules: [Quest.Command.cmdRules.testPosture],
        script() {
            const objs = Quest.Utilities.scopeBy((el) => el.reclineon && el.isAtLoc(Quest.World.player.loc));
            console.log(objs);
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            if (objs.length === 0)
    ...
    new Quest.Command.Cmd('Say', {
        ...
        script(arr) {
            ...
            for (const key in w) {
                // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
                if (Quest.World.w[key].sayCanHear && Quest.World.w[key].sayCanHear(Quest.World.player, arr[0]))
            ...
            if (Quest.Settings.settings.givePlayerSayMsg)
                Quest.IO.msg(Quest.lang.say_something, options);
            for (const chr of l) {
                // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
                if (chr.sayQuestion && Quest.World.w[chr.sayQuestion].sayResponse(chr, arr[1]))
                    return Quest.World.world.SUCCESS;
    ...
    new Quest.Command.Cmd('Give', {
        ...
        matchItems(s) {
            ...
            const npcs = scope.filter((el) => el.npc && el !== Quest.World.player);
            ...
            if (!items)
                return this.setError(Quest.Parser.parser.NO_OBJECT, Quest.lang.object_unknown_msg(arr[0]));
            ...
            if (items[0].length === 1) {
                ...
                if (npcList.length === 0) {
                    // ts-error-fixed ts-migrate(2339) FIXME: Property 'NO_OBJECT' does not exist on type '{}'.
                    if (npcs.length === 0)
                        return this.setError(Quest.Parser.parser.NO_OBJECT, Quest.lang.no_receiver);
                    this.tmp.objects[1] = npcs;
                    this.tmp.objects[0] = items;
                }
            ...
            for (let i = 0; i < this.tmp.objects[0].length; i++) {
                ...
                if (el.length === 1) {
                    ...
                    const held = el.filter((el) => el.loc === Quest.World.player.name);
                    if (held.length === 1) {
    ...
    new Quest.Command.Cmd('NpcGive', {
        ...
        matchItems(s) {
            ...
            if (possibleChars.length === 0)
                return this.setError(Quest.Parser.parser.NO_OBJECT, Quest.lang.object_unknown_msg(charString));
    ...
    new Quest.Command.Cmd('WaitHere', {
        ...
        script(objects) {
            ...
            if (!obj.npc)
                return Quest.IO.falsemsg(Quest.lang.cannot_wait, tpParams);
    ...
    new Quest.Command.Cmd('AskAbout', {
        ...
        rules: [Quest.Command.cmdRules.canTalkTo],
        script(arr) {
            if (!Quest.World.player.testTalk())
                return false;
            if (!arr[0][0].askabout)
                return Quest.IO.failedmsg(Quest.lang.cannot_ask_about, { char: Quest.World.player, item: arr[0][0], text: arr[2] });
    ...
    new Quest.Command.Cmd('TellAbout', {
        ...
        script(arr) {
            ...
            if (!arr[0][0].tellabout)
                return Quest.IO.failedmsg(cannot_tell_about, { char: Quest.World.player, item: arr[0][0], text: arr[1] });
...
if (Quest.Settings.settings.playMode === 'dev') {
    commands.push(new Quest.Command.Cmd('DebugWalkThrough', {
        ...
        script(objects) {
            ...
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            if (wt === undefined)
                return Quest.IO.failedmsg(`No walkthrough found called ${objects[0]}`);
...
function handleFillFromUnknown(char, sink, fluid) {
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'findSource' does not exist on type '{}'.
    if (!Quest.Utilities.util.findSource(options))
        return Quest.IO.failedmsg(fluid ? Quest.lang.no_fluid_here : Quest.lang.no_fluid_here_at_all, options);
...
function handleFillFromVessel(char, source, sink, fluid) {
    ...
    if (!source.vessel)
        return Quest.IO.failedmsg(Quest.lang.not_vessel, options);
    if (source.closed)
    ...
    if (!char.testManipulate(source, 'fill'))
        return Quest.World.world.FAILED;
...
function handleFillFromSource(char, source, sink, fluid) {
    ...
    if (sink.containedFluidName)
        return Quest.IO.failedmsg(Quest.lang.already_full, options);
    if (!char.testManipulate(sink, 'fill'))
        return Quest.World.world.FAILED;
    if (!char.getAgreement('Fill', source, sink, fluid))
        return Quest.World.world.FAILED;
...
export function handleInOutContainer(char, objects, verb, func) {
    ...
    if (container.closed) {
        if (container.containerAutoOpen) {
            if (!container.open({ char, item: container }))
                return false;
        }
    ...
    for (const obj of objects[0]) {
        if (!char.testManipulate(obj, verb))
            return Quest.World.world.FAILED;
...
export function handleSingleDropInContainer(char, container, obj, options) {
    ...
    if (!char.getAgreement('Drop/in', obj, container))
        return;
    ...
    if (obj.testDrop && !obj.testDrop(options))
        return false;
    if (!obj.msgDropIn)
        return Quest.IO.falsemsg(Quest.lang.cannot_drop, options);
    if (container.testDropIn && !container.testDropIn(options))
        return false;
    if (!obj.isAtLoc(char.name))
        return Quest.IO.failedmsg(Quest.lang.not_carrying, { char, item: obj });
    if (obj.getTakeDropCount)
        obj.getTakeDropCount(options, char.name);
...
function handleSingleTakeOutContainer(char, container, obj, options) {
    ...
    if (obj.getTakeDropCount)
        obj.getTakeDropCount(options, container.name);
    if (obj.testTake && !obj.testTake(options))
        return false;
    if (container.testTakeOut && !container.testTakeOut(options))
        return false;
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
...
function handleGiveToNpc(char, objects) {
    ...
    if (success === Quest.World.world.SUCCESS)
        char.pause();
    return success ? Quest.World.world.SUCCESS : Quest.World.world.FAILED;
}
...
function handlePushExit(char, objects) {
    ...
    if (!char.getAgreement('Push', obj, dir))
        return false;
