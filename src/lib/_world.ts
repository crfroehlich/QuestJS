import {
  IGame, IPlayer, IWorld, IWorldClass,
} from '../types/iquest';
...
export const world: IWorldClass = {
  ...
  endTurn(result: any) {
    if (result === true) console.log("That command returned 'true', rather than the proper result code.");
    if (result === false) console.log("That command returned 'false', rather than the proper result code.");
  ...
  initItem(item: any) {
    ...
    if (Quest.Settings.settings.playMode === 'dev') {
      ...
      for (const key in item) {
        ...
        } else if (item[key] instanceof Exit) console.log(`ERROR: Attribute ${key} of ${item.name} is an Exit instance and probably should not.`);
      }
    }
  },
