...
const zoneExit = function (this: any, char: any, exit: any) {
    ...
    if (char.positionX === el.x && char.positionY === el.y && dir === el.dir) {
      ...
      if (el.blocking) {
        return Quest.IO.falsemsg(
          el.blockedmsg || Quest.lang.not_that_way,
          tpParams,
        );
      }
      if (el.isLocked) {
        return Quest.IO.falsemsg(
          el.lockedmsg || Quest.lang.locked_exit,
          tpParams,
        );
      }
  ...
  if (this.origin.defaultToBlocked || this.data.type !== 'compass') {
    return Quest.IO.failedmsg(Quest.lang.not_that_way, { char, dir: this.dir });
  }
  ...
  for (const name in Quest.World.w) {
    ...
    if (
      ...
      return Quest.IO.falsemsg(o.featureNoExit.replace('#', dir));
    }
  }
  ...
  for (const el of this.origin.getBorders()) {
    ...
    if (el.border(newX, newY)) {
      return Quest.IO.falsemsg(el.borderMsg.replace('#', dir));
    }
  }
  ...
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
  msg(Quest.lang.go_successful, { char, dir });
  // ts-error-fixed ts-migrate(7009) FIXME: 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
...
export const ZONE = function (defaultToBlocked: any) {
  ...
  res.hasExit = function (dir: any, options: any) {
    ...
    for (const el of this.zoneExits) {
      ...
      if (
        Quest.World.player.positionX === el.x
        && Quest.World.player.positionY === el.y
        && dir === el.dir
      ) {
        // ("found special")
  ...
  res.desc = function () {
    ...
    for (const el of this.zoneDescs) {
      if (el.when !== undefined) {
        ...
        if (el.when(Quest.World.player.positionX, Quest.World.player.positionY)) {
          return this.getDesc(el);
        }
      } else if (el.x !== undefined) {
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'getDesc' does not exist on type '{ zoneE... Remove this comment to see the full error message
        if (
          el.x === Quest.World.player.positionX
          && el.y === Quest.World.player.positionY
        ) {
          return this.getDesc(el);
        }
      } else {
  ...
  res.getFeatureDescs = function () {
    ...
    for (const name in Quest.World.w) {
      ...
      if (
        Quest.World.player.positionX === el.positionX
        && Quest.World.player.positionY === el.positionY
        && el.featureLookHere
      ) {
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'getBorders' does not exist on type '{ zo... Remove this comment to see the full error message
    for (const el of this.getBorders()) {
      if (el.isAdjacentTo(Quest.World.player) && el.borderDesc) {
        s += ` ${el.borderDesc}`;
      }
    }
    return s;
  ...
  res.beforeEnter = function () {
    if (Quest.World.player.positionX === undefined) {
      Quest.World.player.positionX = 0;
    }
    if (Quest.World.player.positionY === undefined) {
  ...
  res.getDirection = function (char: any, objX: any, objY: any, range: any) {
    ...
    if (x > 0 && theta <= 22.5 && theta >= -22.5) {
      return Quest.lang.exit_list[7].name;
    }
    if (x > 0 && theta <= 67.5 && theta >= 22.5) {
      return Quest.lang.exit_list[2].name;
    }
    if (x > 0 && theta >= -67.5 && theta <= -22.5) {
      return Quest.lang.exit_list[12].name;
    }

    if (x < 0 && theta <= 22.5 && theta >= -22.5) {
      return Quest.lang.exit_list[5].name;
    }
    if (x < 0 && theta <= 67.5 && theta >= 22.5) {
      return Quest.lang.exit_list[10].name;
    }
    if (x < 0 && theta >= -67.5 && theta <= -22.5) {
      return Quest.lang.exit_list[0].name;
    }
  ...
  res.getBorders = function () {
    ...
    for (name in Quest.World.w) {
      ...
      if (
        Quest.World.w[name].zoneBorder
        && Quest.World.w[name].zone === this.name
      ) {
        borders.push(Quest.World.w[name]);
      }
    }
    return borders;
  };
...
export const ZONE_FEATURE = function (
  loc: any,
  x: any,
  y: any,
  range: any,
  ...
  return {
    ...
    isLocatedAt(loc: any) {
      if (adjacent) {
        ...
        return (
          loc === this.zone
          && Math.abs(Quest.World.player.positionX - this.x) < 2
          && Math.abs(Quest.World.player.positionY - this.y) < 2
        );
      }
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'x' does not exist on type '{ positionX: ... Remove this comment to see the full error message
      return (
        loc === this.zone
        && Quest.World.player.positionX === this.x
        && Quest.World.player.positionY === this.y
      );
    },
