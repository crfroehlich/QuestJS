...
const map = {
  defaults: {
    mapDrawPointOfInterest(point: any) {
      ...
      s += `<text x="${
        point.mapX / Quest.Settings.settings.mapScale + 18
      }" y="${point.mapY / Quest.Settings.settings.mapScale - 23}" fill="${
        point.fill
      }">`;
      s += `${point.text}</text>`;
...
map.init = function () {
  ...
  map.questMapDiv.addEventListener('wheel', (e: any) => {
    ...
    Quest.Settings.settings.mapScale -=
      (e.deltaY * -0.01 * Quest.Settings.settings.mapScale) / 4;
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'mapScale' does not exist on type '{ perf... Remove this comment to see the full error message
    Quest.Settings.settings.mapScale = Math.min(
      Math.max(0.2, Quest.Settings.settings.mapScale),
      2.5,
    );
  ...
  map.questMapDiv.addEventListener('mousemove', (e: any) => {
    ...
    // ('@' + (e.offsetX - map.mouseX) + ',' + (e.offsetY - map.mouseY))
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'redraw' does not exist on type '{ toggle... Remove this comment to see the full error message
...
map.redraw = function (offX: any, offY: any) {
  ...
  // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
  if (Quest.World.w[Quest.World.player.loc].mapX)
    Quest.World.player.mapX =
      Quest.World.w[Quest.World.player.loc].mapX /
      Quest.Settings.settings.mapScale;
  // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
  if (Quest.World.w[Quest.World.player.loc].mapY)
    Quest.World.player.mapY =
      Quest.World.w[Quest.World.player.loc].mapY /
      Quest.Settings.settings.mapScale;
  // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
  if (Quest.World.w[Quest.World.player.loc].mapRegion)
    Quest.World.player.mapRegion =
      Quest.World.w[Quest.World.player.loc].mapRegion;
  ...
  if (!Quest.World.player.mapRegion)
    Quest.World.player.mapRegion = Quest.Settings.settings.mapImages[0].name;
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'mapImages' does not exist on type '{ per... Remove this comment to see the full error message
  const mapImage = Quest.Settings.settings.mapImages.find(
    (el: any) => el.name === Quest.World.player.mapRegion,
  );
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  if (!mapImage)
    return Quest.IO.errormsg(
      `Failed to find a map region called '${Quest.World.player.mapRegion}'`,
    );
  ...
  result.push(
    `<image width="${
      mapImage.width / Quest.Settings.settings.mapScale
    }" height="${
      mapImage.height / Quest.Settings.settings.mapScale
  ...
  for (const point of Quest.Settings.settings.mapPointsOfInterest) {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'mapImages' does not exist on type '{ per... Remove this comment to see the full error message
    if (!point.mapRegion)
      point.mapRegion = Quest.Settings.settings.mapImages[0].name;
    ...
    if (!point.isActive || point.isActive())
      result.push(Quest.Settings.settings.mapDrawPointOfInterest(point));
  }
  result.push('</g>');

  // ts-error-fixed ts-migrate(2339) FIXME: Property 'mapOffsetX' does not exist on type '{ pe... Remove this comment to see the full error message
  let offsetX =
    Quest.Settings.settings.mapOffsetX - Quest.Settings.settings.mapWidth / 2;
  if (offX) offsetX += offX;
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'mapOffsetY' does not exist on type '{ pe... Remove this comment to see the full error message
  let offsetY =
    Quest.Settings.settings.mapOffsetY - Quest.Settings.settings.mapHeight / 2;
