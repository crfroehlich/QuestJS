  ...
  export namespace IO {
    ...
    */
    ...
    export const io = {
      ...
      addToOutputQueue: function (data: any) {
        data.id = io.nextid
      ...
      forceOutputFromQueue: function () {
        io.outputSuspended = false
        io.outputFromQueue()
      },

      outputFromQueue: function () {
      ...
      typewriterEffect: function (data: any) {
        if (!data.position) {
          // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
      ...
      unscrambleEffect: function (data: any) {
        // Set up the system
        if (!data.count) {
          // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
          document.querySelector("#output").innerHTML += '<' + data.tag + ' id="n' + data.id + '" class="typewriter"></' + data.tag + '>'
      ...
      unscamblePick: function () {
        let c = String.fromCharCode(Quest.Random.rndm.int(33, 125))
        return c === '<' ? '~' : c
      },

      unscambleScramble: function (data: any) {
        let s = ''
        for (let i = 0; i < data.text.length; i++) {
      ...
      mainGutter: 20,
      panesWidth: 160,
      resizePanesListener: window.matchMedia('(max-width: ' + Quest.Settings.settings.panesCollapseAt + 'px)'),
      resizeMapImageListener: window.matchMedia('(max-width: ' + Quest.Settings.settings.mapAndImageCollapseAt + 'px)'),
      ...
      addClassForClass: function (oldClass: any, newClass: any) {
      ...
      clickExit: function (dir: any) {
        if (io.disableLevel) return
        let failed = false
        Quest.Utilities.runCmd(dir)
      },

      clickItem: function (itemName: any) {
        if (io.disableLevel) return;
      ...
      appendItem: function (item: any, htmlDiv: any, loc: any, isSubItem: any, highlight: any) {
      ...
      getItemHtml: function (item: any, loc: any, isSubItem: any, highlight: any) {
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
      ...
      voice: null,
      voice2: null,
      speak: function (str: any, altVoice: any) {
        if (!io.spoken) return
        if (!io.voice) {
      ...
      showHtml: function (title: any, html: any) {
      ...
      toggleDisplay: function (el: any) {
        if (typeof el === 'string') el = document.querySelector(el)
        el.style.display = el.style.display === 'block' ? 'none' : 'block'
      },

      copyTextToClipboard: function (text: any) {
      ...
      againOrOops: function (isAgain: any) {
      ...
      displayIconsCompass: function (exit: any) {
      ...
      getDropDownText: function (name: any) {
