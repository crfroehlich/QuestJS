    ...
    */
    ...
    function video(filename: any) {
      ...
      _msg('Your browser does not support the <code>video</code> element.', {}, {
        action: 'output', autoplay: true, src: `${Quest.Settings.settings.videoFolder}/${filename}`, tag: 'video',
      });
    }
...
createPanes() {
        ...
        if (Quest.Settings.settings.inventoryPane) {
          for (const inv of Quest.Settings.settings.inventoryPane) {
            html += '<div class="pane-div">';
...
init() {
        ...
        io.createPanes();
        // @ts-expect-error ts-migrate(2339) FIXME: Property 'playMode' does not exist on type '{ perf... Remove this comment to see the full error message
        if (Quest.Settings.settings.playMode === 'play') {
...
unpause: function () {
      ...
      updateStatus() {
        if (Quest.Settings.settings.statusPane) {
          ...
          for (const st of Quest.Settings.settings.status) {
            if (typeof st === 'string') {
      ...
      updateUIItems() {
        if (Quest.Settings.settings.panes === 'none' || !Quest.Settings.settings.inventoryPane) {
          return;
        }
        for (const inv of Quest.Settings.settings.inventoryPane) {
          // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
        ...
        for (const key in Quest.Settings.settings.customPaneFunctions) {
          ...
          if (!el) return;
          // @ts-expect-error ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
          const html   = Quest.Settings.settings.customPaneFunctions[key]();
          el.innerHTML = html;
        }
        io.clickItem('');
      },
