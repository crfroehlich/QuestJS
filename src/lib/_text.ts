  ...
  if (s) {
    ...
    if (typeof text_processors[left] !== 'function') {
      ...
      // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        ...
        // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
        Quest.IO.errormsg(
          `Attempting to use unknown text processor directive '${left}' (${params.tpOriginalString})`,
        );
        return str;
      }
    }
...
const rainbow = function (arr: any, params: any) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    result += `<span style="color:${random.fromArray(colours)}">${s.charAt(
      i,
    )}</span>`;
  }
  return result;
...
const dialogue = function (arr: any, params: any) {
  ...
  if (arr.length < 1)
    Quest.IO.errormsg(
      `Failed to find enough parts in text processor 'dialog' (${params.tpOriginalString})`,
    );
  ...
  if (style.startsWith('.')) {
    ...
    if (arr.length < 2)
      Quest.IO.errormsg(
        `Failed to find enough parts in text processor 'dialog' without a class (${params.tpOriginalString})`,
      );
  ...
  return `${
    prefix +
    Quest.Settings.settings.openQuotation +
    arr.join() +
    Quest.Settings.settings.closeQuotation
  }</span>`;
};
...
const select = function (arr: any, params: any, opt: any) {
  ...
  if (!o)
    Quest.IO.errormsg(
      `Failed to find an object called "${name}" in text processor select.`,
    );
  const l = o[arr[0]];
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  if (l === undefined)
    Quest.IO.errormsg(
      `Failed to find an attribute called "${arr[0]}" for "${name}" in text processor "select" directive.`,
    );
  if (Array.isArray(l)) {
    const n = o[arr[1]];
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    if (!l)
      Quest.IO.errormsg(
        `Failed to find a secondary attribute called "${arr[1]}" for "${name}" in text processor "select" directive.`,
      );
  ...
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  Quest.IO.errormsg(
    `Failed to do anything with the attribute called "${arr[1]}" for "${name}" in text processor select - neither an array or an integer.`,
  );
};
...
const show = function (arr: any, params: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'show' (${params.tpOriginalString})`,
    );
...
const getWhatever = function (val: any, params: any, obj: any) {
  ...
  if (typeof val !== 'function')
    return Quest.IO.errormsg(
      `Got a value of a type I was not expecting in show: ${typeof val}`,
    );
...
const contents = function (arr: any, params: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'contents' (${params.tpOriginalString})`,
    );
...
const number = function (arr: any, params: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'number' (${params.tpOriginalString})`,
    );
  if (typeof obj[arr[0]] === 'number') return Quest.lang.toWords(obj[arr[0]]);
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  return Quest.IO.errormsg(
    `Failed to find a number for object '${name}' in text processor (${params.tpOriginalString})`,
  );
};
...
const money = function (arr: any, params: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'money' (${params.tpOriginalString})`,
    );
  if (obj.loc === Quest.World.player.name && obj.getSellingPrice) {
    return Quest.Utilities.displayMoney(
      obj.getSellingPrice(Quest.World.player),
    );
  }
  ...
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  return Quest.IO.errormsg(
    `Failed to find a price for object '${name}' in text processor (${params.tpOriginalString})`,
  );
};
...
const transitDest = function (arr: any, params: any) {
  ...
  const transit = arr[0]
    ? Quest.World.w[arr[0]]
    : Quest.World.w[Quest.World.player.loc];
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  if (!transit.transitDoorDir)
    return Quest.IO.errormsg(
      `Trying to use the 'transitDest' text process directive when the player is not in a transit location (${params.tpOriginalString}).`,
    );
...
const img = function (arr: any, params: any) {
  const src = arr[0].includes('/')
    ? arr[0]
    : Quest.Settings.settings.imagesFolder + arr[0];
...
const handleIf = function (arr: any, params: any, reverse: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'if/ifNot' (${params.tpOriginalString})`,
    );
  name = arr.shift();
  let attValue =
    typeof obj[name] === 'function' ? obj[name](params) : obj[name];
  ...
  if (typeof attValue === 'boolean') {
    ...
    if (typeof attValue === 'number') {
      ...
      if (isNaN(value))
        return Quest.IO.errormsg(
          `Trying to compare a numeric attribute, '${name}' with a string (${params.tpOriginalString}).`,
        );
...
const handleIfExists = function (arr: any, params: any, reverse: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'if/ifNotExists' (${params.tpOriginalString})`,
    );
  name = arr.shift();
...
const handleIfLessMoreThan = function (
  arr: any,
  params: any,
  moreThan: any,
  orEqual: any,
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'ifLessMoreThan' (${params.tpOriginalString})`,
    );
  name = arr.shift();
  const attValue =
    typeof obj[name] === 'function' ? obj[name](params) : obj[name];
  ...
  if (typeof attValue !== 'number')
    return Quest.IO.errormsg(
      `Trying to use ifLessThan with a non-numeric (or nonexistent) attribute, '${name}' (${params.tpOriginalString}).`,
    );
  let value = arr.shift();
  // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  if (isNaN(value))
...
const handleIfHere = function (
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'ifHere' (${params.tpOriginalString})`,
    );
...
const handleIfPlayer = function (arr: any, params: any, reverse: any) {
  ...
  if (!obj)
    return Quest.IO.errormsg(
      `Failed to find object '${name}' in text processor 'if/ifNotPlayer' (${params.tpOriginalString})`,
    );
...
const terse = function (arr: any, params: any) {
  if (
    (Quest.Settings.settings.verbosity === Quest.World.world.TERSE &&
      Quest.World.currentLocation.visited === 0) ||
...
const hereDesc = function (arr: any, params: any) {
  ...
  if (typeof room.desc === 'string') {
    ...
    if (s === undefined) {
      Quest.IO.errormsg(
        `This room description is not set up properly. It has a 'desc' function that does not return a string. The room is "${room.name}".`,
        true,
      );
...
const exits = function (arr: any, params: any) {
  // ts-error-fixed ts-migrate(7053) FIXME: Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
  const list = Quest.World.w[Quest.World.player.loc].getExitDirs();
  return Quest.Utilities.formatList(list, {
    lastJoiner: Quest.lang.list_or,
    nothing: Quest.lang.list_nowhere,
  });
};
...
const nameFunction = function (arr: any, params: any, isPossessive: any) {
  ...
  return arr[1] === 'true'
    ? Quest.Utilities.sentenceCase(Quest.lang.getName(subject, options))
    : Quest.lang.getName(subject, options);
};
...
const handlePronouns = function (arr: any, params: any, pronoun: any) {
  ...
  if (!subject) return false;
  return arr[0] === 'true'
    ? Quest.Utilities.sentenceCase(subject.pronouns[pronoun])
...
const pa2 = function (arr: any, params: any) {
  ...
  if (chr1.pronouns === chr2.pronouns && chr1 !== chr2) {
    ...
    return arr[0] === 'true'
      ? Quest.Utilities.sentenceCase(Quest.lang.getName(chr1, opt))
      : Quest.lang.getName(chr1, opt);
  }

  return arr[0] === 'true'
    ? Quest.Utilities.sentenceCase(chr1.pronouns.poss_adj)
...
const pa3 = function (arr: any, params: any) {
  ...
  if (chr1 !== chr2) {
    ...
    return arr[0] === 'true'
      ? Quest.Utilities.sentenceCase(Quest.lang.getName(subject, opt))
      : Quest.lang.getName(subject, opt);
  }
