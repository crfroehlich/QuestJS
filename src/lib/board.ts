...
board.update = function () {
  ...
  if (this.settings.compassPane)
    this.map.push(
      board.getCompass(this.settings.compass.x, this.settings.compass.y),
    );
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  ...
  // ts-error-fixed ts-migrate(2531) FIXME: Object is possibly 'null'.
  document.querySelector('#board').innerHTML = `<svg width="${
    this.settings.width
  }" height="${this.settings.height}" viewBox="0 0 ${this.settings.width} ${
...
board.handleCell = function (x: any, y: any) {
  ...
  s += '" ';
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  s += board.settings.cellBorder
    ? board.settings.cellBorder(x, y)
    : 'stroke="none"';
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  ...
  if (typeof this.settings.getLeftBorder === 'function') {
    ...
    if (leftBorder) {
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
      s += `<line x1="${x2 + this.settings.cellPoly[0][0]}" y1="${
        y2 + this.settings.cellPoly[0][1]
      }" `;
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
      s += `x2="${x2 + this.settings.cellPoly[3][0]}" y2="${
        y2 + this.settings.cellPoly[3][1]
      }" ${leftBorder}/>`;
    }
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
    ...
    if (rightBorder) {
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
      s += `<line x1="${x2 + this.settings.cellPoly[2][0]}" y1="${
        y2 + this.settings.cellPoly[2][1]
      }" `;
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
      s += `x2="${x2 + this.settings.cellPoly[3][0]}" y2="${
        y2 + this.settings.cellPoly[3][1]
  ...
  for (const el of this.settings.getFeaturesAt(x, y)) {
    ...
    if (feature.script) {
      ...
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
      s += `width="${Math.round(
        this.settings.cellSize * this.settings.rootTwo,
      )}"`;
...
board.getCompass = function (x: any, y: any) {
  ...
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  s += `" transform="scale(1, ${
    this.settings.cosAngle / this.settings.sinAngle
  })" transform-origin="`;
  s += `${x}px ${y}px"/>`;
  return s;
};
...
board.getSwitches = function () {
  ...
  if (this.settings.switchesWidth) {
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
    s += `${
      this.settings.switches.length * 18 + 14
...
board.getBase = function () {
  ...
  s += `${
    ...
    this.settings.sinAngle * this.settings.size * this.settings.cellSize +
    this.settings.cellPoly[0][0]
  },`;
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  s += `${
    this.settings.height / 2 +
    this.settings.offsetY +
    this.settings.cosAngle * this.settings.size * this.settings.cellSize
  } `;
  ...
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  s += `${
    this.settings.height / 2 + this.settings.offsetY + this.settings.baseHeight
  } `;
  ...
  s += `${
    this.settings.offsetX +
    this.settings.sinAngle * this.settings.size * this.settings.cellSize * 2 +
    this.settings.cellPoly[0][0]
  },`;
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  s += `${this.settings.height / 2 + this.settings.offsetY} `;
...
board.getCoord = function (x: any, y: any) {
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  const x2 =
    this.settings.cellSize * (x + y) * this.settings.sinAngle +
    this.settings.offsetX;
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'settings' does not exist on type '{}'.
  const y2 =
    this.settings.height / 2 +
    this.settings.cellSize * (x - y) * this.settings.cosAngle +
    this.settings.offsetY;
  return [x2, y2];
};
