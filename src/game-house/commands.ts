...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('ThrowAtPod', {
  ...
  script(objects: any) {
    ...
    obj.moveToFrom({ item: obj, char: Quest.World.player }, "greenhouse_east")
    // @ts-expect-error ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('Shift', {
  ...
  script: function (objects) {
    const obj = objects[0][0]
    const tpParams = { item: obj, char: Quest.World.player }
...
Quest.Commands.commands.unshift(new Quest.Command.Cmd('BurstBalloonOnly', {
  ...
  script: function (objects) {
    ...
    if (obj.smash) return objects[0][0].smash({ item: obj, char: Quest.World.player }) ? Quest.World.world.SUCCESS : Quest.World.world.FAILED
