...
// ts-error-fixed ts-migrate(2339) FIXME: Property 'output' does not exist on type '{ perfor... Remove this comment to see the full error message
    ...
    Quest.Settings.settings.afterTurn.push(() => {
      for (const key in Quest.World.w) {
        ...
        if (obj.activeEffects) {
          for (const name of obj.activeEffects) {
    ...
    export const rpg = {
      ...
      broadcast(group: any, message: any, source: any, other: any) {
        for (const key in Quest.World.w) {
      ...
      broadcastAll(message: any, source: any, other: any) {
        log(source.name);
        for (const key in Quest.World.w) {
          const o = Quest.World.w[key];
      ...
      broadcastCommunication(npc: any, message: any, source: any, other: any) {
      ...
      defaultSkillAfterUse(attack: any, count: any) { },

      defaultSkillTestUseable(char: any) {
        return true;
      },

      defaultSpellAfterUse(attack: any, count: any) { },
      defaultSpellTestUseable(char: any) {
        return true;
      },
      ...
      getAll(target: any) {
        return rpg.handleGetting(target, () => true, true);
      },

      getAllBut(target: any) {
        return rpg.handleGetting(target, () => true, false);
      },
      ...
      getFoes(target: any) {
        return rpg.handleGetting(target, (o: any) => o.allegiance !== 'friend', true);
      },

      getFoesBut(target: any) {
        return rpg.handleGetting(target, (o: any) => o.allegiance !== 'friend', false);
      },

      // Get a list of hostiles in the current room.
      // May not work without a parameter to isHostile.
      getHostiles(target: any) {
        return rpg.handleGetting(target, (o: any) => o.isHostile(), true);
      },

      getHostilesBut(target: any) {
        return rpg.handleGetting(target, (o: any) => o.isHostile(), false);
      },

      handleGetting(target: any, fn: any, includeTarget: any) {
        const l = Quest.Utilities.scopeHereListed().filter((el) => el.npc && fn(el) && el !== target);
        if (target !== undefined && includeTarget) l.unshift(target);
        return l;
      },

      hasEffect(obj: any, effect: any) {
        if (!obj.activeEffects) return false;
        if (typeof effect !== 'string') effect = effect.name;
        return obj.activeEffects.includes(effect);
      },

      isSpellAvailable(char: any, spell: any) {
        for (const el of Quest.Utilities.scopeHeldBy(char)) {
      ...
      signalResponse_test(source: any) {
