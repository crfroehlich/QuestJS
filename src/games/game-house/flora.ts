...
Quest.World.createRoom('greenhouse_west', {
  ...
  up: new Quest.World.Exit('_', {
    ...
    simpleUse(char: any) {
      ...
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
      if (Quest.World.w.grown_tamarind_tree.growthTime < 9) return Quest.IO.falsemsg('Mandy looks at the {nm:item:false:false:count_this} thoughtfully. If {nv:item:were} more substantial, she could climb {ob:item}.', { count_this: 'seedsPlanted', item: Quest.World.w.grown_tamarind_tree });
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'grown_tamarind_tree' does not exist on t... Remove this comment to see the full error message
...
Quest.World.createItem('grown_tamarind_tree', {
  ...
  magicGrowth() {
    ...
    this.update();
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
    if (Quest.World.player.loc === 'greenhouse_west') Quest.IO.msg('Mandy watches as {nv:item:grow:false:count_this}; {nv:item:be:false:suppressCount} now about {show:grown_tamarind_tree:getHeight} high.', { count_this: 'seedsPlanted', item: Quest.World.w.grown_tamarind_tree, suppressCount: 'seedsPlanted' });
  },
  magicGrowthEnd() {
    if (this.seedsPlanted === 0) return;
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 3 arguments, but got 2.
