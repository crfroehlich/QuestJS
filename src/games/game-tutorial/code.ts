...
export const init = (Quest: QuestClass) => {
  function tmsg(s: any, params: any) {
    msg(s, params || {}, { cssClass: 'tutorial', tag: 'p' });
  }
  ...
  hint.data = [
    ...
    {
      hint: 'TURN ON TORCH, then head to the basement (out, southwest, then down). If you are in the basement and it is dark, you will need to go back up, and then turn the torch on before coming back down.',
      name: 'torchOn',
      tutorial:
    ...
    },
    {
      ...
      name: 'save',
      tutorial:
        "Not much in this room, so let's pause for a moment. It is a good idea to save occasionally whilst playing, just in case you die (not possible in this game) or lose the connection to the server (not an issue for Quest 6) or your PC crashes or you just have some else to do and want to return later. You really do not want to have to start from the beginning, so save your game.|Different systems have different ways to handle saving and loading (and some games may not support it at all), but a good start is to type SAVE.",
    },
    {
      hint: 'Type SAVE TUTORIAL (or some other name).',
      name: 'saveGame',
      tutorial:
        'So in Quest 6 SAVE just tells you how to save your game. You need to add a file name to actually save. Do that now! You can call it whatever you want; how about "tutorial"?',
    },
    ...
    },
    {
      ...
      name: 'askRLift',
      tutorial:
        'Something is wrong... Perhaps we should ask the robot about it?',
    },
    ...
    {
      hint: 'Tell the robot to go north (ROBOT,N), then go NORTH yourself and tell the robot to get the rod (ROBOT,GET ROD).',
      name: 'rGoNorth',
      tutorial:
    ...
    },
    {
      hint() {
        ...
        // ts-error-fixed ts-migrate(2339) FIXME: Property 'me' does not exist on type '{}'.
          ...
          Quest.World.w.me.loc === 'laboratory' ||
          Quest.World.w.me.loc === 'lounge'
        ) {
          // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
    ...
    {
      hint: 'Look for the code for the computer. Try LOOK BEHIND PAINTING, then read what you find. Once you have the number, try to USE THE COMPUTER again.',
      name: 'findCode',
      tutorial:
    ...
    {
      hint: 'You need to escape, and the only way out is through the window. What happens if you SMASH THE WINDOW?',
      name: 'smashWindow',
      tutorial: '',
    },
    {
      hint: 'You need to escape, and the only way out is through the window, but you need to wrap your hand in something first. How about that old newspaper (WRAP FIST IN NEWSPAPER)?',
      name: 'wrapFist',
      tutorial: '',
    },
    {
      hint: 'You need to escape, and the only way out is through the window. What happens if you SMASH THE WINDOW now your fist is wrapped in newspaper?',
      name: 'smashWindow2',
      tutorial: '',
    },
    {
      hint: 'You need to escape, and the only way out is through the broken window. What happens if you try OUT?',
      name: 'out',
      tutorial: '',
    },
    {
      hint: 'You need to escape, and the only way out is through the broken window, but it is a long way down and you will need to use the rope. To do that, TIE Quest.Templates.ROPE TO DESK, then THROW Quest.Templates.ROPE OUT WINDOW.',
      name: 'climbOut',
    },
  ...
  findCmd('MetaHint').script = function () {
    ...
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'data' does not exist on type '{}'.
      ...
      typeof hint.data[Quest.World.player.hintCounter].hint === 'function'
    ) {
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'data' does not exist on type '{}'.
      hint.data[Quest.World.player.hintCounter].hint();
    } else {
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'data' does not exist on type '{}'.
      log(hint.data[Quest.World.player.hintCounter].name);
      // ts-error-fixed ts-migrate(2339) FIXME: Property 'data' does not exist on type '{}'.
      log(
        `hint.data[Quest.World.player.hintCounter].hint is a ${typeof hint.data[
          Quest.World.player.hintCounter
        ].hint}`,
      );
    }
  ...
  const wrapScript = function (obj1: any, obj2: any) {
    ...
    if (obj2 !== Quest.World.w.old_newspaper)
      return Quest.IO.failedmsg('You cannot wrap that round anything.');
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'fist' does not exist on type '{}'.
    if (obj1 !== Quest.World.w.fist)
      return Quest.IO.failedmsg("You don't think that will achieve anything.");
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  ...
  const unwrapScript = function (obj1: any, obj2: any) {
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'old_newspaper' does not exist on type '{... Remove this comment to see the full error message
    if (obj2 !== Quest.World.w.old_newspaper)
      return Quest.IO.failedmsg('They are not wrapped together.');
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'fist' does not exist on type '{}'.
    if (obj1 !== Quest.World.w.fist)
      return Quest.IO.failedmsg('They are not wrapped together.');
    // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
  ...
  Quest.Commands.commands.unshift(
    new Cmd('ThrowThrough', {
      ...
      regex:
        /^(?:throw|chuck|hurl|toss|pitch|lob|heave) (.+) (?:out of|out|through) (.+)$/,
      script(objects: any) {
        ...
        if (!item.isAtLoc('me'))
          return Quest.IO.failedmsg('You are not holding {nm:item:the}.', {
            item,
          });
  ...
  const smashWithScript = function (item: any, dest: any) {
    ...
    if (dest !== Quest.World.w.office_window)
      return Quest.IO.failedmsg("That's not something you can smash.");
    if (!item.isAtLoc('me'))
      return Quest.IO.failedmsg('You are not holding {nm:item:the}.', { item });
    // ts-error-fixed ts-migrate(2339) FIXME: Property 'office_window' does not exist on type '{... Remove this comment to see the full error message
    ...
    if (item === Quest.World.w.crowbar) {
      ...
      if (Quest.World.w.Professor_Kleinscope.isHere())
        msg(
    ...
    msg("You can't smash the window using {nm:item:the}.", { item });
      ...
      script(objects: any) {
        if (objects[0][0].npc) {
          ...
          if (!Quest.World.w.me.killFlag) {
            // ts-error-fixed ts-migrate(2554) FIXME: Expected 2 arguments, but got 1.
            tmsg(
              'You will find most games will not let you attack the characters, and those that do will probably have combat as a large part of the game. That said, it is a good idea to try these things, you never know quite what will happen.',
            );
            // ts-error-fixed ts-migrate(2339) FIXME: Property 'me' does not exist on type '{}'.
  ...
  Quest.Commands.commands.push(
    new Cmd('RudeCommand', {
      ...
      // throw rope out window
      regex:
        /^(?:fuck|facefuck|face-fuck|face fuck|bugger|shag|suck|suck off|assfuck|ass-fuck|ass fuck|rape|ass-rape|ass rape) (.+?)$/,
      script(objects: any) {
  ...
  /*
  Quest.Commands.commands.unshift(new Cmd('ThrowAt', {
