...
export const init = (Quest: QuestClass) => {
  ...
  Quest.IO.io.msgInputText = function (s: any) {
    ...
    document.querySelector('#output').innerHTML += `<p id="n${
      Quest.IO.io.nextid
    }" class="input-text">&gt; ${s.toUpperCase()}</p>`;
    Quest.IO.io.nextid++;
  ...
  const roster = {
    data: [
      {
        alias: 'Helm',
        desc: 'The officer at the helm has responsibility for piloting the ship, and needs a good skill at navigation.',
        name: 'helm',
        skill: 'navigation',
      },
      {
        alias: 'Chief Engineer',
        desc: 'The chief engineer has the job of ensuring all systems on the ship are running smoothly, working with the 28 ratings who make up most of the crew.',
        name: 'engineering',
        skill: 'engineering',
      },
      {
        alias: 'Science Officer',
        desc: 'The science officer leads a team of twelve, who are trained in various disciplines. He or she will report findings and opinions to the captains, as appropriate.',
        name: 'science',
        skill: 'science',
      },
      {
        alias: 'Armsman',
        desc: "The armsman is responsible for targeting and firing the ship's weapons in the event of combat, which requires skill with the weapon as well as the ability to remain calm under pressure. The office will be in command of eight crewman who are responsible for maintenance of the weapon systems (so knowledge of engineering is of limited use). The officer is also in charge of the marines and shipboard security.",
        name: 'armsman',
        skill: 'weapons',
      },
    ],
    ...
    getOfficer(pos: any) {
      return this.hasOfficer(pos)
        ? Quest.World.w[Quest.World.w.ship[pos]]
        : null;
    },
  ...
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
  Quest.Text.addDirective('sir', (arr: any, params: any) =>
    Quest.World.player.callmemaam ? "ma'am" : 'sir',
  );
  // ts-error-fixed ts-migrate(2339) FIXME: Property 'addDirective' does not exist on type '{ ... Remove this comment to see the full error message
  Quest.Text.addDirective('Sir', (arr: any, params: any) =>
    Quest.World.player.callmemaam ? "Ma'am" : 'Sir',
